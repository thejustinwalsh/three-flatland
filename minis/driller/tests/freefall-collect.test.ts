...
describe('free-fall click-to-collect', () => {
  it('clicking far from a gem during free-fall still collects it (no distance check)', () => {
    ...
    const gemEntity = world.spawn(
      Gem({ col: 5, row: 160, color: 'emerald', size: 'medium', collected: false, scatteredUntilTick: 0 }),
    )
  ...
  it('returns NEAREST gem when multiple are present', () => {
    ...
    const near = world.spawn(
      Gem({ col: 4, row: 160, color: 'emerald', size: 'small', collected: false, scatteredUntilTick: 0 }),
    )
    world.spawn(
      Gem({ col: 15, row: 200, color: 'topaz', size: 'small', collected: false, scatteredUntilTick: 0 }),
    )
  ...
  it('does NOT bypass exact-cell rule when driller is in the body (not free-fall)', () => {
    ...
    world.spawn(
      Gem({ col: 5, row: 12, color: 'emerald', size: 'medium', collected: false, scatteredUntilTick: 0 }),
    )
  ...
  it('exact-cell click on the gem still works in free-fall', () => {
    ...
    const gemEntity = world.spawn(
      Gem({ col: 7, row: 155, color: 'ruby', size: 'small', collected: false, scatteredUntilTick: 0 }),
    )
