...
describe('integration: 3-phase sag state machine timing', () => {
  ...
  it('phase durations match design (within ±60ms p50)', async () => {
    const { data, log } = await runProbe<ThreePhaseResult>('./probes/three-phase-timing.probe.js', {
      timeoutSec: 150,
    })
    ...
    expect(data.saggingToShaking.n, 'No SAGGING→SHAKING transitions observed.').toBeGreaterThan(10)
    expect(data.shakingToRelease.n, 'No SHAKING→release transitions observed.').toBeGreaterThan(5)

    assertPhaseP50('PRECARIOUS → SAGGING', data.precariousToSagging.p50, TARGET_PRECARIOUS_MS, log)
    assertPhaseBand(
      'SAGGING → SHAKING',
    ...
    assertPhaseP50('SHAKING → release', data.shakingToRelease.p50, TARGET_SHAKING_MS, log)
  })
})
