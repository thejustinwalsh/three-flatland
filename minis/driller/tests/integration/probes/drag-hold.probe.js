...
if (!anchor) {
  ...
  console.log('[probe] found cluster cid=', anchor.cid, 'size=', anchor.size, 'at', anchor.col, anchor.row)
}
...
while (Date.now() < heldUntil) {
  ...
  if (now - progressAt >= 250) {
    progressAt = now
    console.log(`[progress] drag held ${now - (heldUntil - HOLD_MS)}ms heldViolations=${heldViolations}`)
  }
