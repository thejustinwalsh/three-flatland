...
import { collapseTick, detectAndSag, tickFalling, tickSagging } from '../src/systems/collapse'
...
import { SAG_PRECARIOUS_TICKS, SAG_SAGGING_TICKS, TILE_PX } from '../src/constants'
...
describe('detectAndSag', () => {
  it('does NOT sag a soil chunk that is sitting on bedrock with no room to fall', () => {
    ...
    const world = makeWorldFromGrid(['.....', '..#..', '..#..', '..S..', 'SSSSS'])
  ...
  it('does not sag a chunk where every cantilever-unstable cell has solid below', () => {
    ...
    // the spawn.
    const world = makeWorldFromGrid(['.................', '#################', 'SSSSSSSSSSSSSSSSS'])
...
describe('collapseTick (sag → fall release)', () => {
  it('cancels a sag whose path is blocked at shake entry', () => {
    const world = makeWorldFromGrid(['.....', '.##..', '..S..', 'SSSSS'])
    const grid = world.get(Grid)!
    const shakeStart = SAG_PRECARIOUS_TICKS + SAG_SAGGING_TICKS
    grid.flags[grid.cols + 1] = FLAG_SAGGING
    grid.flags[grid.cols + 2] = FLAG_SAGGING
    world.spawn(
      SaggingChunk({
        cells: [
          { col: 1, row: 1, tile: TILE_SOIL },
          { col: 2, row: 1, tile: TILE_SOIL },
        ],
        startTick: 0,
        durationTicks: shakeStart + 30,
      })
    )
    world.set(GameState, { tick: shakeStart })
    ...
    world.query(SaggingChunk).forEach(() => sagCount++)
    expect(sagCount).toBe(0)
    expect(grid.flags[grid.cols + 1]! & FLAG_SHAKING).toBe(0)
    expect(grid.flags[grid.cols + 2]! & FLAG_SHAKING).toBe(0)
  })
...
describe('falling chunk collision', () => {
  it('sweeps crossed rows and crushes the driller instead of tunneling through support', () => {
    const world = makeWorldFromGrid(['......', '......', '......', '......', '######', '......'])
    world.spawn(
      Driller({
        col: 2,
        row: 3,
        px: 2 * TILE_PX + TILE_PX / 2,
        py: 3 * TILE_PX + TILE_PX / 2,
        destCol: 2,
        destRow: 3,
      })
    )
    world.spawn(
      FallingChunk({
        cells: [{ col: 0, row: 0, tile: TILE_SOIL }],
        px: 2 * TILE_PX,
        py: TILE_PX,
        vy: TILE_PX * 1.5,
        releaseRow: 1,
    ...
    expect(grid.tiles[3 * grid.cols + 2]).toBe(TILE_SOIL)
    expect(grid.tiles[4 * grid.cols + 2]).toBe(TILE_SOIL)
    let fallingCount = 0
    world.query(FallingChunk).forEach(() => fallingCount++)
    expect(fallingCount).toBe(0)
  })
})
