...
import {
  ...
  planGreedy,
  plannerTick,
  type HazardReactionStyle,
} from '../src/systems/ai-planner'
import { drillerSystem } from '../src/systems/driller'
import { Animation, Driller, GameState, Gem, Hazard, Mood, PlannerTarget } from '../src/traits'
...
describe('warning-phase hazard reactions', () => {
  it('briefly waits, then commits to one escape side for the whole telegraph', () => {
    ...
    const fallAtTick = findFallAtTick('wait', 5, 8)
    world.spawn(
      Hazard({
        col: 5,
        py: TILE_PX,
    ...
    plannerTick(world)
    expect(driller.get(PlannerTarget)).toMatchObject({ col: 5, row: 8 })

    world.set(GameState, { tick: 30 })
    plannerTick(world)
    const committedCol = driller.get(PlannerTarget)!.col
    expect(Math.abs(committedCol - 5)).toBeGreaterThanOrEqual(2)

    const step = Math.sign(committedCol - 5)
    driller.set(Driller, {
      col: 5 + step,
      px: (5 + step) * TILE_PX + TILE_PX / 2,
      destCol: 5 + step,
    })
    world.set(GameState, { tick: 31 })
    plannerTick(world)

    expect(driller.get(PlannerTarget)!.col).toBe(committedCol)
  })

  it('panic-drills through a side wall instead of pacing under the warning', () => {
    const world = makeWorldFromGrid([
      '..........',
      '..........',
      '..........',
    ...
    const fallAtTick = findFallAtTick('panic', 5, 8)
    world.spawn(
      Hazard({
        col: 5,
        py: TILE_PX,
    ...
    expect(Math.abs(target.col - 5)).toBeGreaterThanOrEqual(2)

    drillerSystem(world, 1000 / 60)

    expect(driller.get(Driller)!.drillCooldownMs).toBeGreaterThan(0)
    expect(['drillLeft', 'drillRight']).toContain(driller.get(Animation)!.state)
  })
})
...
describe('planGreedy — drills through stones', () => {
  ...
  it('drills a stone straight down instead of pacing toward a cheaper side route', () => {
    // Depth drive is a hard greedy priority. A stone takes 4 hits while
    // the side route costs only 2 actions, but repeatedly optimizing that
    // local cost made Homie wander horizontally despite a clear drillable
    // route below. Greedy now means down first; only fixtures and hazards
    // justify searching sideways.
    const rows = [
      '..........',
      '..........',
      '..........',
...
function findFallAtTick(style: HazardReactionStyle, col: number, row: number): number {
  for (let tick = 90; tick < 300; tick++) {
    if (hazardReactionStyle(col, row, tick) === style) return tick
  }
  throw new Error(`Could not find a deterministic ${style} hazard reaction`)
}
