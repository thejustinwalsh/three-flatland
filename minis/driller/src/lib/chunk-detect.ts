import { ANCHOR_DIST_INF, isFixtureTile, TILE_AIR, TILE_SOIL, TILE_STONE } from '../traits'
...
export function relaxAnchorDist(
  ...
  for (let i = 0; i < tiles.length; i++) {
    ...
    const c = i % cols
    const r = (i - c) / cols
    const isSeed = r === topRow || (r > 0 && isFixtureTile(tiles[(r - 1) * cols + c]!))
    let target: number
    if (isSeed) {
      target = 0
    } else {
 ...
 * legacy contract).
 */
export function anchorDistanceMap(tiles: Uint8Array, cols: number, rows: number): Int32Array {
 ...
 */
export function unstableCells(tiles: Uint8Array, dist: Uint8Array, maxReach: number): Set<number> {
