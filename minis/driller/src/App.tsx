...
/**
 * Inline ZzFX implementation for standalone dev mode.
 * Based on https://github.com/KilledByAPixel/ZzFX
 *
 * Mirrors the implementation in `minis/breakout/src/App.tsx` so the dev
 * experience is consistent across minis.
 */
function createZzfx(): PlaySoundFn {
  ...
  return (...params: ZzFXParams) => {
    ...
    for (let i = 0; i < length; i++) {
      if (i < attackTime) d = i / attackTime
      else if (i < sustainTime) d = 1
      else if (i < releaseTime) d = 1 - (i - sustainTime) / (release * sampleRate)
      else d = 0
      ...
      let sample = 0
      if (shape === 0) sample = Math.sin(t)
      else if (shape === 1) sample = Math.sin(t) > 0 ? 1 : -1
      else if (shape === 2) sample = ((t / PI2) % 1) * 2 - 1
      else if (shape === 3) sample = 1 - Math.abs(((t / PI2) % 1) * 2 - 1) * 2
      else if (shape === 4) sample = Math.random() * 2 - 1
