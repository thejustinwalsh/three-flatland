...
function resizeUint32(source: Uint32Array, length: number, carry: number): Uint32Array {
  const next = new Uint32Array(length)
  next.set(source.subarray(0, carry))
  return next
}

function resizeInt32(source: Int32Array, length: number, carry: number): Int32Array {
...
export function ensureDebugRenderState(tilesLength: number): DebugRenderState | undefined {
  ...
  if (!state) {
    ...
    state.shakeFramesGrid = resizeUint32(state.shakeFramesGrid, tilesLength, carry)
    state.shakeFramesRendered = resizeUint32(state.shakeFramesRendered, tilesLength, carry)
    state.lastRenderedFrame = resizeInt32(state.lastRenderedFrame, tilesLength, carry)
  }
  return state
}
