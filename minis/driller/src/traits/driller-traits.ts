...
export const Driller = trait({
  ...
  /**
   * Brief post-fall lock in milliseconds. The renderer uses the dedicated
   * `land` state to play the final impact frames once while this counts down.
   */
  landingStunMs: 0,
  /** Snapped row where the current drop began; -1 while grounded. */
  fallStartRow: -1,
})
