...
import { CHUNK_ROWS, PLAY_COLS, STONE_MAX_HITS } from '../constants'
import {
  Explosive,
  FLAG_AUTOTILE_DIRTY,
  Gem,
...
export interface GeneratedChunk {
  ...
  /**
   * Stone indices that should spawn pre-damaged (one drill from breaking).
   * Speed-bump stones start at `STONE_MAX_HITS - 1` hits on these
...
const STONE_SHAPES: { weight: number; cells: StoneShape }[] = [
  ...
  // anchors in mid-air under the cantilever rule, so they're
...
function pickStoneShape(rng: Rng, biome: string): StoneShape {
  ...
  const biasFactor =
    biome === 'stoneworks' || biome === 'crystal-caverns' || biome === 'core' ? 0.7 : 1
...
export function generateChunk(seed: number, chunkY: number): GeneratedChunk {
  ...
  for (let i = 0; i < rockBudget; i++) {
    ...
    const idx = y * cols + x
    // Rocks use stone tiles, but speed-bump
  ...
  for (let i = 0; i < gemCount; i++) {
    for (let attempt = 0; attempt < GEM_PLACEMENT_RETRIES; attempt++) {
      ...
      if (tiles[idx] === TILE_SOIL || tiles[idx] === TILE_AIR) {
        ...
        const size: GemSize =
          sizeRoll < 0.5 ? 'small' : sizeRoll < 0.8 ? 'medium' : sizeRoll < 0.95 ? 'large' : 'huge'
  ...
  for (let r = 0; r < rows; r++) {
    ...
    for (let i = 0; i < attempts; i++) {
      ...
      const sizeRoll = rng.next()
      const size: GemSize =
        sizeRoll < 0.45 ? 'small' : sizeRoll < 0.75 ? 'medium' : sizeRoll < 0.92 ? 'large' : 'huge'
...
function loadChunk(world: World, chunkY: number, seed: number): void {
  ...
  for (let r = 0; r < CHUNK_ROWS; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      frameIndex[dst] = 0
      // Hits represent damage taken. Fresh stones from
