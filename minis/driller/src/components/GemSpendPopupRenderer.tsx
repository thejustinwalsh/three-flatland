...
const DIGIT_FRAMES: Record<string, ReturnType<typeof frameFromRegion>> = {}
for (const [k, v] of Object.entries(DIGIT_REGIONS)) {
  DIGIT_FRAMES[k] = frameFromRegion(v, DIGIT_SHEET_W, DIGIT_SHEET_H)
}
const GEM_FRAME = frameFromRegion(ICON_REGIONS.gem, ICON_SHEET_W, ICON_SHEET_H)
...
export function GemSpendPopupRenderer({ iconsMaterial, digitsMaterial }: Props) {
  ...
  useEffect(() => {
    for (const s of iconRefs.current) if (s) s.scale.set(0, 0, 1)
    for (const s of digitRefs.current) if (s) s.scale.set(0, 0, 1)
  }, [])
