...
function pickTint(tile: number, frame: number, sagging: boolean, falling: boolean): readonly [number, number, number] {
  ...
  useFrame(() => {
    ...
    for (let r = topRow; r < bottomRow && slot < POOL_SIZE; r++) {
      for (let c = 0; c < cols && slot < POOL_SIZE; c++) {
        ...
        const tint = pickTint(tile, frameIndex[idx]!, sagging, falling)
        sprite.visible = true
        sprite.position.set(c * TILE_PX + TILE_PX / 2, -(r * TILE_PX + TILE_PX / 2), 0)
        sprite.scale.set(TILE_PX, TILE_PX, 1)
        // Mutate the existing Color reference component-wise so the
        // observable proxy fires on each channel write.
