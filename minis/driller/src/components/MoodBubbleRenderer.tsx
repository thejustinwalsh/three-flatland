...
import { OVER_PET_THRESHOLD, OVER_PET_WINDOW_TICKS, PET_PAUSE_TICKS, TILE_PX } from '../constants'
import {
...
const FRAMES = {
  'pet.love': rectToFrame(ICON_REGIONS['pet.love'], ICON_SHEET_W, ICON_SHEET_H),
  'pet.warning': rectToFrame(ICON_REGIONS['pet.warning'], ICON_SHEET_W, ICON_SHEET_H),
}
...
export function MoodBubbleRenderer({ iconsMaterial, bubbleMaterial }: Props) {
  ...
  useFrame(() => {
    ...
    const inWindow = pe
      ...
      : 0
    // Affection stays visible until the over-pet warning threshold.
    //
    ...
    let iconName: keyof typeof FRAMES
