 ...
 *                     mood axes, signals
 *   actions         — deterministic runtime actions for browser QA
 *   render          — anchor-heatmap toggle, future debug-render buttons
 *
...
export function DebugPanel() {
  ...
  useEffect(() => {
    ...
    ai.addBinding(ref, 'animation', { readonly: true, label: 'animation' })
    ...
    // ---- actions (collapsed) ----
    const actions = bundle.pane.addFolder({ title: 'actions', expanded: false })
    actions.addButton({ title: 'trigger death sequence' }).on('click', () => {
      const gs = world.get(GameState)
      if (!gs || gs.runState === 'dying') return
      world.set(GameState, { runState: 'dying' })
    })

    // ---- render (collapsed) ----
    const render = bundle.pane.addFolder({ title: 'render', expanded: false })
    render.addButton({ title: 'toggle anchor heatmap' }).on('click', () => {
  ...
  useFrame(() => {
    ...
    if (dEntity) {
      ...
      ref.animation = dEntity.get(Animation)?.state ?? '—'
