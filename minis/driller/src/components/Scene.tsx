...
import { Camera, GameState, type RunState } from '../traits'
...
import { hazardSpawnSystem, hazardTickSystem, rockAvalancheSystem } from '../systems/hazard'
...
extend({ DefaultLightEffect, Flatland, Light2D, NormalMapProvider, Sprite2D, Sprite2DMaterial })

declare global {
  interface Window {
    __drillerFlat?: FlatlandType
  }
}
...
export function Scene({ onShellStateChange }: SceneProps) {
  ...
  const renderer = useThree((state): unknown => state.renderer)
  const size = useThree((s) => s.size)
  const material = useDrillerMaterial()
  const worldTileMaterial = useWorldTileMaterial()
  const gemMaterial = useGemMaterial()
  ...
  useFrame((_, delta) => {
    ...
    if (cam) {
      ...
      // Three.js scene graph nodes are intentionally mutated during the render loop.
  ...
  useFrame(
    (state) => {
      ...
      if (!flatland || !isWebGPURenderer(renderer)) return
        ...
        {lightingEnabled && (
          <>
            <defaultLightEffect
              ...
              bands={5}
              lightHeight={TILE_PX * 1.5}
              shadowStrength={0.42}
              shadowBias={0.75}
              shadowStartOffsetScale={0.8}
              shadowMaxDistance={TILE_PX * 7}
              categoryQuotas={{ gem: 2, crystal: 2 }}
            />
            <DrillerLighting />
          </>
        )}
        <TileRenderer material={worldTileMaterial} />
        <FallingChunkView material={worldTileMaterial} />
...
/** Query switch used by visual regression tools to capture an unlit baseline. */
function readLightingEnabledFromUrl(): boolean {
  if (typeof window === 'undefined') return true
  return new URLSearchParams(window.location.search).get('lighting') !== '0'
}
...
function syncRenderState(
  ...
  if (cam && flatland) {
    ...
    if (typeof window !== 'undefined') window.__drillerFlat = flatland
  }
}
