...
export function HoverOutlineRenderer({ outlineMaterial, fillMaterial }: Props) {
  ...
  useFrame((_, delta) => {
    ...
    const placeOutline = (col: number, row: number, tintHex: string, pulse = false) => {
      ...
      if (!sprite) return
      const s = pulse ? TILE_PX * (1 + 0.25 * (0.5 + 0.5 * Math.sin(now * Math.PI * 3.2))) : TILE_PX
    ...
    const placePerimeter = (cells: Iterable<{ col: number; row: number }>, tintHex: string) => {
      const set = new Set<number>()
      const cellKey = (col: number, row: number) => row * PLAY_COLS + col
      for (const cell of cells) set.add(cellKey(cell.col, cell.row))
      const inSet = (col: number, row: number) => set.has(cellKey(col, row))
      for (const key of set) {
        const c = { col: key % PLAY_COLS, row: Math.floor(key / PLAY_COLS) }
    ...
    <>
      {Array.from({ length: OUTLINE_POOL }).map((_, i) => (
        <sprite2D
          ...
          scale={[0, 0, 1]}
          sortLayer={RENDER_LAYERS.interaction}
          lit={false}
        />
      ))}
