  ...
  return (
    ...
    >
      ...
      {!saved ? (
        ...
        <>
          <div style={{ fontSize: 11, opacity: 0.7, textTransform: 'uppercase', letterSpacing: '0.1em' }}>
            saved
          </div>
          <div style={{ background: 'rgba(0,0,0,0.4)', padding: 12, borderRadius: 8, fontSize: 12, minWidth: 240 }}>
            {entries.slice(0, 5).map((e, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ opacity: e.name === name.trim() ? 1 : 0.7 }}>{i + 1}. {e.name}</span>
                <span>{e.depthM}m · ◆{e.gems}</span>
              </div>
            ))}
          </div>
          <button
