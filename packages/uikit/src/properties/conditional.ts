...
export type Conditionals = Record<
  Exclude<(typeof SpecialLayerSections)[number], 'important'>,
  () => boolean
>
...
function createHoverConditionals(
  hoveredSignal: Signal<Array<number>>
): Pick<Conditionals, 'hover'> {
  return {
...
function createActivePropertyTransfomers(
  activeSignal: Signal<Array<number>>
): Pick<Conditionals, 'active'> {
  return {
...
function createFocusPropertyTransformers(
  hasFocusSignal?: Signal<boolean>
): Pick<Conditionals, 'focus'> {
