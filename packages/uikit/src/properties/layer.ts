...
export const SpecialLayerSections = Object.keys(SectionStartIndexMap).filter(
  (layer) => layer != 'base'
) as Array<Exclude<keyof typeof SectionStartIndexMap, 'base'>>

export function getLayerIndex(identifier: LayerIdentifier) {
  if (
    identifier.type != 'class' &&
    identifier.type != 'default-overrides' &&
  ...
  if (classIndex >= MaxClassAmount) {
    throw new Error(
      `class index "${classIndex}" exceeds the maximum number of classes (${MaxClassAmount})`
    )
  }
