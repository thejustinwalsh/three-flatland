...
export function useSetup(ref: { current: Component | null }, inProps: any, args: Array<any>): any {
  ...
  // The a11y projection follows R3F's ACTIVE camera — the one the app renders with. A custom-render
  // app must make its render camera R3F's active camera (`useThree().set({ camera })`); it then flows
  // here with no per-root configuration.
