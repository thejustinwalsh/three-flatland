...
export function reversePainterSortStable(
  ...
  if (aRootSignal != null && aRootSignal.peek() === bRootSignal?.peek()) {
    return compareOrderInfo(
      (a.object as any)[orderInfoKey].value,
      (b.object as any)[orderInfoKey].value
    )
  }
...
export function setupOrderInfo(
  ...
  abortableEffect(() => {
    ...
    const majorIndex = parseNumberValue(
      properties.value[zIndexKey as 'zIndex'] ?? basisOrderInfo?.majorIndex ?? 0
    )
...
function shallowEqualRecord(
  r1: Record<string, any> | undefined,
  r2: Record<string, any> | undefined
): boolean {
  if (r1 === r2) {
    return true
  }
