...
import { YogaProperties, setMeasureFunc, setter } from '../flex/index.js'
import { createDefaultConfig } from '../flex/yoga.js'
...
function getRawValue(property: string, node: Node): any {
  for (const propertyWithEdge of propertiesWithEdge) {
    if (property.startsWith(propertyWithEdge)) {
      const edgeProperty = property.match(
        /^(border|padding|margin|position)(Top|Bottom|Right|Left)(?:Width)?$/
      )
...
describe('set & get properties', () => {
  ...
  it('it should throw an error', () => {
    expect(
      () => setter.alignItems(root, node, 'centerx' as any),
    ...
    expect(
      () => setter.alignItems(root, node, 1 as any),
  ...
  it('it should reset all values', () => {
    ;(Object.keys(testValues) as Array<keyof YogaProperties>).forEach((name) =>
      setter[name](root, node, undefined)
    )
    properties.forEach((property) => {
      expect(
  ...
  it('accepts representative schema-valid values in the Yoga setters', () => {
    ...
    >) {
      for (const value of values) {
        expect(
          yogaPropertyShape[key].safeParse(value).success,
          `${key} schema accepts ${value}`
        ).to.equal(true)
        expect(
          () => setter[key](root, node, value as never),
          `${key} accepts ${value}`
        ).to.not.throw()
      }
    }
