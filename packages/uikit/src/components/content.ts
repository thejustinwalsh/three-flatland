...
import { ElementType, type OrderInfo, setupOrderInfo, setupRenderOrder } from '../order.js'
...
> extends Component<OutProperties> {
  ...
  constructor(
    ...
    this.boundingBox =
      inputConfig?.boundingBox ??
      signal<BoundingBox>({ size: new Vector3(1, 1.01, 1), center: new Vector3(0, 0, 0) })
    ...
    abortableEffect(() => {
      ...
      scaleHelper
        .set(
          ...
          this.properties.value.keepAspectRatio
            ? (innerHeight * pixelSize * this.boundingBox.value.size.z) /
                this.boundingBox.value.size.y
      ...
      positionHelper.z -=
        alignmentZMap[this.properties.value.depthAlign] * this.boundingBox.value.size.z
    ...
    const remeasureOnChildrenChange =
      this.inputConfig?.remeasureOnChildrenChange ?? RemeasureOnChildrenChangeDefault
  ...
  private childUpdateWorldMatrix(child: Object3D, updateParents: boolean, updateChildren: boolean) {
    ...
    child.matrixWorld
      .multiply(this.globalMatrix.peek() ?? IdentityMatrix)
      .multiply(this.childrenMatrix)
