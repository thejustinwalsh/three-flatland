...
import type { BaseOutProperties, InProperties, Properties } from '../properties/index.js'
import { conditionalKeys } from '../properties/conditional.js'
import type { LayerInSectionIdentifier } from '../properties/layer.js'
  ...
  *[Symbol.iterator]() {
  ...
  set(...classes: typeof this.list): void {
    ...
    for (let classIndex = 0; classIndex < length; classIndex++) {
      ...
      const resolvedClass = classRef == null ? undefined : this.resolveClassRef(classRef)
...
export function getStarProperties<T extends BaseOutProperties>(
  properties: InProperties<T> | undefined
) {
  if (properties == null) {
    return undefined
  }
