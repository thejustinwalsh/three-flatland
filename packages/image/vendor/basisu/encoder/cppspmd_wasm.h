// cppspmd_wasm.h — WebAssembly SIMD128 port of cppspmd_sse.h.
// Copyright 2020-2022 Binomial LLC (original SSE version)
// Path B port: replaces SSE intrinsics with wasm_simd128 via sse_to_wasm.h shim.
//
...
//
// Notes for Basis Universal Path B:
// Near-verbatim copy of cppspmd_sse.h with the namespace renamed to cppspmd_wasm
// and the x86 intrinsic headers replaced by sse_to_wasm.h. The shim defines
// `__m128`, `__m128i`, `__m128d` as `v128_t` and provides inline implementations
// for every `_mm_*` intrinsic this file uses.
...
// SSE→wasm compatibility shim — defines __m128/__m128i/__m128d and the
// `_mm_*` intrinsics this file uses on top of wasm_simd128.h.
#include "sse_to_wasm.h"
...
// wasm port: pretend to be SSE4.1-class (we have shuffles, blendv, min/max_epu32, etc.).
// Use the same _sse41 arch suffix as the SSE port so the symbols emitted by
// basisu_kernels_imp.h (e.g. `perceptual_distance_rgb_4_N_sse41`) match what
// callers in basisu_etc.cpp / basisu_frontend.cpp / basisu_backend.cpp expect.
// Namespace also matches so `using namespace CPPSPMD;` resolves consistently.
...
} // namespace cppspmd_sse41 (wasm port)
