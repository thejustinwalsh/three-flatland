...
describe('vendor-data-theme-rewrite', () => {
    describe('positive `[data-theme=…]` predicates', () => {
        it('rewrites :root[data-theme="light"] to @media + :root', () => {
            ...
            expect(norm(out)).toBe(
                norm(`@media (prefers-color-scheme: light) { :root { --x: 1; } }`)
            )
        })
        ...
        it('rewrites html[data-theme="light"] to @media + html', () => {
            ...
            expect(norm(out)).toBe(
                norm(`@media (prefers-color-scheme: light) { html { color-scheme: light; } }`)
            )
        })
        ...
        it('strips [data-theme="light"] from descendant selector', () => {
            ...
            expect(norm(out)).toBe(
                norm(`@media (prefers-color-scheme: light) { .foo { color: red; } }`)
            )
        })
        ...
        it('rewrites dark predicate to @media (prefers-color-scheme: dark)', () => {
            ...
            expect(norm(out)).toBe(
                norm(`@media (prefers-color-scheme: dark) { :root { --x: 1; } }`)
            )
        })
    })
    ...
    describe('comma-separated selector lists', () => {
        it('keeps multi-selector light block together', () => {
            const out = rewrite(
                `:root[data-theme='light'], [data-theme='light'] ::backdrop { --x: 1; }`
            )
            expect(norm(out)).toBe(
                norm(
                    `@media (prefers-color-scheme: light) { :root, ::backdrop { --x: 1; } }`
                )
            )
        })
        ...
        it('partitions a list with both light + dark themes', () => {
            const out = rewrite(
                `[data-theme='light'] .x, [data-theme='dark'] .y { color: red; }`
            )
            const normalized = norm(out)
            expect(normalized).toContain(
                norm(`@media (prefers-color-scheme: light) { .x { color: red; } }`)
            )
            expect(normalized).toContain(
                norm(`@media (prefers-color-scheme: dark) { .y { color: red; } }`)
            )
        })

        it('separates untheme selectors from themed ones', () => {
            const out = rewrite(
                `.a, :root[data-theme='light'] .b { color: red; }`
            )
            const normalized = norm(out)
            expect(normalized).toContain(norm(`.a { color: red; }`))
            expect(normalized).toContain(
                norm(`@media (prefers-color-scheme: light) { :root .b { color: red; } }`)
            )
        })
    })
    ...
    describe('composite predicates within a single selector', () => {
        it('keeps consistent theme combo (light root + light-equivalent descendant)', () => {
            const out = rewrite(
                `:root[data-theme='light'] .x:not([data-theme='dark']) { --y: 1; }`
            )
            expect(norm(out)).toBe(
                norm(`@media (prefers-color-scheme: light) { :root .x { --y: 1; } }`)
            )
        })

        it('drops contradictory theme combo (root=light + descendant=dark)', () => {
            const out = rewrite(
                `:root:not([data-theme='dark']) .expressive-code[data-theme='dark'] .ec { color: red; }`
            )
        ...
        it('keeps a per-element forced-light selector under @media light', () => {
            const out = rewrite(
                `.expressive-code[data-theme='light'] .ec { color: red; }`
            )
            expect(norm(out)).toBe(
                norm(`@media (prefers-color-scheme: light) { .expressive-code .ec { color: red; } }`)
            )
        })
    })
