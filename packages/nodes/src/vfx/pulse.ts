import { vec2, vec3, vec4, float, sin, mix } from 'three/tsl'
import type Node from 'three/src/nodes/core/Node.js'
import type { FloatInput, Vec3Input, Vec2Input } from '../types'
...
export function pulseHeartbeat(
  ...
  const pulseVal = beat1.add(beat2).clamp(0, 1).mul(intensityNode)
  const brightness = float(1).add(pulseVal)
...
export function pulseRainbow(
  ...
  const mixed = mix(inputColor.rgb, rainbow, satNode)
