 ...
 * @param distance - Maximum light distance (default: 100)
 * @param decay - Decay exponent controlling attenuation curve shape (default: 2 for quadratic)
...
export function pointLight2D(
  ...
  const distanceNode = typeof distance === 'number' ? float(distance) : distance
 ...
 * @param distance - Maximum light distance
...
export function spotLight2D(
  ...
  const distAttenuation = float(1).sub(dist.div(distanceNode).clamp(0, 1))
