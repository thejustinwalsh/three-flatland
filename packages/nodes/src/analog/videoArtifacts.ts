import { vec2, vec3, vec4, float, floor, fract, mix, sin } from 'three/tsl'
...
export function colorBleeding(
  ...
  let colorSum: Node<'vec3'> = tex.sample(uv).rgb
...
export function staticNoise(
  ...
  const mixed = mix(inputColor.rgb, staticColor, intensityNode)
