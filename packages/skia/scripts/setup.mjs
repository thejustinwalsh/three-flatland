...
import { existsSync, mkdirSync, readFileSync, writeFileSync, readdirSync, chmodSync, cpSync, rmSync } from 'node:fs'
import { resolve, dirname, relative, delimiter } from 'node:path'
...
// Freshness stamp: a hash of the WASM sources recorded next to the built libs.
// CI commits it alongside lib/*.wasm so a fresh checkout knows the committed
// libs match the committed sources and can skip the (expensive) recompile. It's
// an internal build artifact — tracked in git for the freshness check, but not
// in `files` (consumers don't need it).
const WASM_STAMP = resolve(PKG_ROOT, 'lib/.wasm-sources.sha256')

// Inputs that determine the compiled WASM, mirroring the wasm-specific entries
// of skia's nx `build` target (TS/`production` deliberately excluded — a pure-TS
// change must NOT force a multi-minute WASM recompile). The skia submodule is
// proxied by its git tree SHA rather than hashing its thousands of files.
const WASM_SOURCE_FILES = ['build.zig', 'build.zig.zon']
const WASM_SOURCE_DIRS = ['src/zig', 'patches', 'vendor', 'wit']

// Deterministic content hash of the WASM sources. Walks dirs in sorted order and
// folds each file's repo-relative path + bytes into the digest, so the result is
// stable across machines and reruns.
function wasmSourcesHash() {
  ...
  const hashDir = (dir) => {
    ...
    for (const entry of entries) {
      ...
      hash.update(relative(PKG_ROOT, full))
      if (entry.isDirectory()) hashDir(full)
      else hash.update(readFileSync(full))
    }
  }
  for (const file of WASM_SOURCE_FILES) {
    const full = resolve(PKG_ROOT, file)
    if (existsSync(full)) {
      hash.update(file)
      hash.update(readFileSync(full))
    }
  }
  for (const dir of WASM_SOURCE_DIRS) hashDir(resolve(PKG_ROOT, dir))
  try {
    const sha = execSync('git rev-parse HEAD:third_party/skia', {
      cwd: PKG_ROOT,
      stdio: ['ignore', 'pipe', 'ignore'],
    })
      .toString()
      .trim()
    if (sha) hash.update(`submodule:${sha}`)
  } catch {
    // Not a git checkout (or submodule not gitlinked) — the file-content hash of
    // the tracked sources still gates freshness; the submodule SHA is a bonus.
  }
  return hash.digest('hex')
}

// Do the committed libs match the current WASM sources? Only then is a recompile
// unnecessary. Existence alone is NOT enough (that was the staleness bug): a
// skia-source change must invalidate the committed libs.
function wasmFresh(variants) {
  const libsExist = variants.every((v) => existsSync(resolve(PKG_ROOT, `lib/skia-${v}.wasm`)))
  if (!libsExist || !existsSync(WASM_STAMP)) return false
  return readFileSync(WASM_STAMP, 'utf8').trim() === wasmSourcesHash()
}
...
async function downloadGithubRelease(repo, version, binName, { archOverride, expectedSha256 } = {}) {
  ...
  const asset = release.assets?.find(
    (a) => a.name.includes(`${effectiveArch}-${platform}`) && a.name.endsWith('.tar.gz') && !a.name.endsWith('.sha256')
  )
  ...
  if (!asset) {
    ...
    if (release.message) {
      console.log(
        `  GitHub API: ${release.message}${GH_TOKEN ? '' : ' (requests are unauthenticated — set GITHUB_TOKEN)'}`
      )
    }
    return false
  }
  ...
  if (binName === 'wasm-opt') {
    ...
    if (libDir && existsSync(libDir)) {
      ...
      const libs = execSync(`ls "${libDir}"/libbinaryen*`, { stdio: 'pipe' }).toString().trim().split('\n')
...
async function installTools(config) {
  ...
  if (verifyToolVersion('wasm-opt', tools.binaryen.version)) {
    ...
    const success = await downloadGithubRelease(tools.binaryen.repo, `version_${tools.binaryen.version}`, 'wasm-opt', {
...
async function main() {
  ...
  // Debug aid: print the WASM-sources hash and exit. Handy for diagnosing why CI
  // did (or didn't) recompile — compare against lib/.wasm-sources.sha256.
  if (args.includes('--wasm-hash')) {
    console.log(wasmSourcesHash())
    return
  }

  // --ensure: if the committed libs already match the current WASM sources,
  // there's nothing to do — skip the whole submodule/tools/compile pipeline.
  // Source-aware (not just "libs exist"), so a skia-source change forces the
  // recompile that keeps CI's committed libs honest. No toolchain needed to
  // consume fresh libs, so this is also the macOS fast path.
  if (ensure) {
    const variants = glOnly ? ['gl'] : wgpuOnly ? ['wgpu'] : ['gl', 'wgpu']
    if (wasmFresh(variants)) {
      return // committed libs are current for these sources
    }
  }

  console.log('')
  console.log(`${C.bold}${C.magenta}  ╔══════════════════════════════════════════╗${C.reset}`)
  console.log(`${C.bold}${C.magenta}  ║    @three-flatland/skia — WASM Setup     ║${C.reset}`)
  console.log(`${C.bold}${C.magenta}  ╚══════════════════════════════════════════╝${C.reset}`)
  ...
  if (!canBuildWasm()) {
    const variants = glOnly ? ['gl'] : wgpuOnly ? ['wgpu'] : ['gl', 'wgpu']
    const libsPresent = variants.every((v) => existsSync(resolve(PKG_ROOT, `lib/skia-${v}.wasm`)))
    if (libsPresent) {
      ok("Can't compile WASM on this host — using the committed lib/*.wasm (CI rebuilds on skia changes).")
      return
    }
    fail(
      "Committed skia lib/*.wasm are missing and this host can't compile them — restore them (git) or build on Linux/CI. NOT fetching a remote prebuilt."
    )
    process.exit(1)
  }
  ...
  // 4. Build WASM. We only reach here when the libs are NOT fresh (the --ensure
  // gate returned early otherwise), so always compile — never skip on mere
  // existence. Record the sources' hash so the next --ensure can skip cleanly.
  buildWasm(glOnly, wgpuOnly)
  writeFileSync(WASM_STAMP, `${wasmSourcesHash()}\n`)

  // Done
  heading('Done')
  ok('Skia WASM build complete!')
  info(
    `Test: ${C.dim}npx serve . -p 3333${C.reset} then open ${C.dim}http://localhost:3333/test/browser-test.html${C.reset}`
  )
  console.log('')
}
