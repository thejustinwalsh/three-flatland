...
console.log(
  `  actualBoundingBoxLeft=${m.actualBoundingBoxLeft.toFixed(2)} Right=${m.actualBoundingBoxRight.toFixed(2)}`
)
console.log(
  `  actualBoundingBoxAscent=${m.actualBoundingBoxAscent.toFixed(2)} Descent=${m.actualBoundingBoxDescent.toFixed(2)}`
)
console.log(
  `  fontBoundingBoxAscent=${m.fontBoundingBoxAscent.toFixed(2)} Descent=${m.fontBoundingBoxDescent.toFixed(2)}`
)
...
console.log(
  `  actualBoundingBoxLeft=${bm.actualBoundingBoxLeft.toFixed(2)} Right=${bm.actualBoundingBoxRight.toFixed(2)}`
)
console.log(
  `  actualBoundingBoxAscent=${bm.actualBoundingBoxAscent.toFixed(2)} Descent=${bm.actualBoundingBoxDescent.toFixed(2)}`
)
console.log(
  `  fontBoundingBoxAscent=${bm.fontBoundingBoxAscent.toFixed(2)} Descent=${bm.fontBoundingBoxDescent.toFixed(2)}`
)
...
for (const ch of 'Lorem ipsum dolor sit') {
  ...
  const g = glyphs.get(id)
  if (!g) {
    console.log(`  "${ch}" NO GLYPH`)
    continue
  }
  console.log(
    `  "${ch}" id=${id} yMin=${g.bounds.yMin.toFixed(3)} yMax=${g.bounds.yMax.toFixed(3)} ascent@48=${(g.bounds.yMax * 48).toFixed(2)}  curves=${g.curves.length}`
  )
}
