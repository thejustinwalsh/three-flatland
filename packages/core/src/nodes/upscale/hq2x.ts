import { vec2, vec3, vec4, float, floor, mix, texture as sampleTexture } from 'three/tsl'
...
export function hq2x(
  ...
  const diff = (a: Node<'vec4'>, b: Node<'vec4'>) => {
    const aYUV = yuv(a)
    const bYUV = yuv(b)
  ...
  const _subX = localPos.x.mul(2).fract()
  const _subY = localPos.y.mul(2).fract()

  // Blend toward edges when appropriate
  const topLeftColor = p1Blend.select(mix(w5.rgb, w4.rgb.add(w2.rgb).mul(0.5), float(0.5)), w5.rgb)

  const topRightColor = p2Blend.select(mix(w5.rgb, w2.rgb.add(w6.rgb).mul(0.5), float(0.5)), w5.rgb)

  const bottomLeftColor = p3Blend.select(mix(w5.rgb, w4.rgb.add(w8.rgb).mul(0.5), float(0.5)), w5.rgb)

  const bottomRightColor = p4Blend.select(mix(w5.rgb, w6.rgb.add(w8.rgb).mul(0.5), float(0.5)), w5.rgb)
...
export function hq4x(
  ...
  // Sample neighborhood
  const _w1 = sampleTexture(tex, srcUV.add(texel.mul(vec2(-1, -1))))
  const w2 = sampleTexture(tex, srcUV.add(texel.mul(vec2(0, -1))))
  const _w3 = sampleTexture(tex, srcUV.add(texel.mul(vec2(1, -1))))
  const w4 = sampleTexture(tex, srcUV.add(texel.mul(vec2(-1, 0))))
  const w5 = sampleTexture(tex, srcUV)
  const w6 = sampleTexture(tex, srcUV.add(texel.mul(vec2(1, 0))))
  const _w7 = sampleTexture(tex, srcUV.add(texel.mul(vec2(-1, 1))))
  const w8 = sampleTexture(tex, srcUV.add(texel.mul(vec2(0, 1))))
  const _w9 = sampleTexture(tex, srcUV.add(texel.mul(vec2(1, 1))))
  ...
  const topRow = mix(topLeft, topRight, subX)
  const bottomRow = mix(bottomLeft, bottomRight, subX)
  const _result = mix(topRow, bottomRow, subY)
  ...
  // Blend toward center
  const blended = mix(finalResult, w5.rgb, float(0.3))
