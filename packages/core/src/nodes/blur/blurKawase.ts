import { vec2, float, texture as sampleTexture } from 'three/tsl'
...
export function blurKawaseSimple(
  ...
  // Center sample
  let result: Node<'vec4'> = sampleTexture(tex, uv).mul(4)
...
export function blurKawaseUp(
  ...
  let result: Node<'vec4'> = sampleTexture(tex, uv.add(halfTexel.mul(vec2(-1, -1)))).mul(2)
...
export function blurKawaseMulti(
  ...
  let result: Node<'vec4'> = sampleTexture(tex, uv)
  let totalWeight: Node<'float'> = float(1)
