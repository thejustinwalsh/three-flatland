import { vec2, float, sin, floor, fract } from 'three/tsl'
 ...
 * @internal
 */
function valueNoise2D(p: Node<'vec2'>): Node<'float'> {
  const i = floor(p)
  const f = fract(p)
 ...
 * @internal
 */
function grad2D(hash: Node<'float'>, p: Node<'vec2'>): Node<'float'> {
 ...
 * @internal
 */
function perlinNoise2D(p: Node<'vec2'>): Node<'float'> {
  const i = floor(p)
  const f = fract(p)
...
export function distortPerlin(
  ...
  // Accumulate octaves
  let noiseX: Node<'float'> = float(0)
  let noiseY: Node<'float'> = float(0)
  let amplitude: Node<'float'> = float(1)
  let frequency: Node<'float'> = float(1)
  let totalAmplitude: Node<'float'> = float(0)
...
export function distortTurbulence(
  ...
  let noiseY: Node<'float'> = float(0)
  let amplitude: Node<'float'> = float(1)
  let frequency: Node<'float'> = scaleNode
  let totalAmplitude: Node<'float'> = float(0)
