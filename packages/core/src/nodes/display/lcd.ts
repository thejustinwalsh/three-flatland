import { vec3, vec4, float, floor, mix, mod } from 'three/tsl'
...
export function dotMatrix(
  input: Node<'vec4'> | TextureNode<'vec4'>,
  uv: Node<'vec2'>,
  ...
  const pixelColor = mix(bgVec, inputColor.rgb, inPixel)
...
export function lcdGhosting(
  currentTex: TextureNode<'vec4'>,
  previousTex: TextureNode<'vec4'>,
  uv: Node<'vec2'>,
  ...
  const rising = diff.x.add(diff.y).add(diff.z).greaterThan(0)
  ...
  const blended = mix(previous.rgb, current.rgb, float(1).sub(factor))
...
export function lcdPocket(
  ...
  const color = mix(dark, light, contrasted.mul(dotMask))
