import { vec2, vec3, vec4, float, sin } from 'three/tsl'
 ...
 */
export function crtCurvature(uv: Node<'vec2'>, curvature: FloatInput = 0.1) {
...
export function crtBloom(
  ...
  let bloom: Node<'vec3'> = vec3(0, 0, 0)
...
export function crtComplete(tex: TextureNode<'vec4'>, uv: Node<'vec2'>, options: CRTOptions = {}) {
  const {
    curvature = 0.1,
