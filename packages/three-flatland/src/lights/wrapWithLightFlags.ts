import { int, attribute, select, vec2 } from 'three/tsl'
 ...
 * interleaved core buffer and pure-effect buffers. Keep every named
 * field readable through a helper so shader code doesn't repeat the
 * underlying attribute name + component index, and the packed layout
 * stays refactorable in one place.
 *
 ...
 * Used by {@link wrapWithLightFlags} to gate the light pipeline;
 * custom ColorTransforms can also call this directly.
 */
 ...
 *
 * Uses `select()` to bypass lighting for sprites whose lit flag is
 * not set. Only needed for batched sprites — standalone usage can
 * assign the raw `lightFn` directly.
 *
