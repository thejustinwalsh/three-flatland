      ...
      defaultOverrides: {
        ...
        minWidth: computed(() =>
          Math.max(
            ...
            2 * sliderProcessPaddingXs[this.properties.value.size ?? 'md'] +
              (this.thumb.size.value?.[0] ?? 0)
          )
        ),
        width: percentage,
        height: '100%',
    ...
    this.labels.add(
      (this.leftLabel = new Text(undefined, undefined, {
        defaultOverrides: {
          text: computed(
            () =>
              this.properties.signal.leftLabel.value ??
              format.value(Number(this.properties.value.min ?? 0))
          ),
        },
      })),
      (this.rightLabel = new Text(undefined, undefined, {
        defaultOverrides: {
          text: computed(
            () =>
              this.properties.signal.rightLabel.value ??
              format.value(Number(this.properties.value.max ?? 100))
          ),
        },
      }))
    )
