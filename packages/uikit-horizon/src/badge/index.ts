...
export class Badge extends Container<BadgeOutProperties> {
  ...
  constructor(
    ...
    super(inputProperties, initialClasses, {
      ...
      defaultOverrides: {
        borderRadius: computed(() =>
          (this.properties.value.label?.length ?? 0) > 1 || this.properties.value.icon != null
            ? 8
            : 1000
        ),
        color: computed(
          () => badgeVariants[this.properties.value.variant ?? 'primary'].color?.value
        ),
    ...
    this.iconPlaceholder = new Container(undefined, undefined, {
      defaultOverrides: {
        display: computed(() => (this.properties.value.icon == null ? 'none' : 'flex')),
      },
    })
    ...
    this.label = new Text(undefined, undefined, {
      defaultOverrides: {
        ...
        display: computed(() =>
          (this.properties.value.label?.length ?? 0) === 0 ? 'none' : 'flex'
        ),
      },
    })
