...
const _avatarSizes = {
  xl: {
    attributionActiveMargin: 4,
    attributionActiveWidth: 24,
    attributionSrcWidth: 44,
    borderWidth: 3,
    height: 120,
  },
  lg: {
    attributionActiveMargin: 2,
    attributionActiveWidth: 16,
    attributionSrcWidth: 32,
    borderWidth: 3,
    height: 72,
  },
  md: {
    attributionActiveMargin: 0,
    attributionActiveWidth: 12,
    attributionSrcWidth: 16,
    borderWidth: 2,
    height: 44,
  },
...
export class Avatar extends Container<AvatarOutProperties> {
  ...
  constructor(
    ...
    //focus ring
    const focusRingBorderWidth = computed(
      () => avatarSizes[this.properties.value.size ?? 'md'].borderWidth!
    )
    super.add(
    ...
    //avatar image
    super.add(
      (this.image = new Image<ImageOutProperties<string | Texture>>(
    ...
    super.add(
      (this.activeBadge = new Container(undefined, undefined, {
        defaultOverrides: {
          display: computed(() =>
            (this.properties.value.attributionActive ?? false) &&
            this.properties.value.attributionSrc == null
              ? 'flex'
              : 'none'
          ),
          zIndex: 1,
    ...
    abortableEffect(() => {
      ...
      const attributionSrcWidth = computed(
        () => avatarSizes[this.properties.value.size ?? 'md'].attributionSrcWidth
      )
      const attributionImage = new Image<ImageOutProperties<string | Texture>>(
        undefined,
        undefined,
        {
          defaultOverrides: {
