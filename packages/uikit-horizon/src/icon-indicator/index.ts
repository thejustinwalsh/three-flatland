...
const iconIndicatorVariants = _iconIndicatorVariants as UnionizeVariants<
  typeof _iconIndicatorVariants
>
...
export type IconIndicatorOutProperties = BaseOutProperties &
  z.output<typeof IconIndicatorOutPropertiesSchema>
    ...
    super.add(
      (this.icon = new Svg(undefined, undefined, {
        defaultOverrides: {
          ...
          height: 20,
          content: computed(
            () => iconIndicatorVariants[this.properties.value.variant ?? 'none'].content
          ),
          color: computed(
            () => iconIndicatorVariants[this.properties.value.variant ?? 'none'].color?.value
          ),
        },
      }))
    )
