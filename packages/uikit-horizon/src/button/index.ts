...
type ButtonVariantProps = Pick<
  ContainerProperties,
  'backgroundColor' | 'hover' | 'color' | 'active' | 'important'
>
...
export class Button extends Container<ButtonOutProperties> {
  constructor(
    ...
    super(inputProperties, initialClasses, {
      ...
      defaultOverrides: {
        color: computed(
          () => buttonVariants[this.properties.value.variant ?? 'primary'].color?.value
        ),
        ...
        hover: {
          color: computed(
            () => buttonVariants[this.properties.value.variant ?? 'primary'].hover?.color?.value
          ),
          backgroundColor: computed(
            () =>
              buttonVariants[this.properties.value.variant ?? 'primary'].hover?.backgroundColor
                ?.value
          ),
        },
        active: {
          color: computed(
            () => buttonVariants[this.properties.value.variant ?? 'primary'].active?.color?.value
          ),
          backgroundColor: computed(
            () =>
              buttonVariants[this.properties.value.variant ?? 'primary'].active?.backgroundColor
                ?.value
          ),
        },
        important: {
