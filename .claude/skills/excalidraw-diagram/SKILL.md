...
**Omit meaningless bookends.** Do NOT add terminal "start" and "end" nodes — the rounded `( start )` / `( end )` circles, or context-only labels like `render`, `canvas`, `Sprite atlas`, `input`, `output`, `user`, `done`. The reader already knows the surrounding context; these nodes carry no information, waste space, and pad the diagram's footprint. Begin the diagram at the first node that actually *does* something and end it at the last meaningful step. A pipeline starts at its first real stage and ends at its last real stage — not at a decorative terminal. (Genuine intermediate artifacts that the pipeline transforms — a buffer, a render target, a distance field — are real nodes and stay; the rule is specifically about content-free entry/exit terminals.)
...
- `--strict` — exit non-zero (3) if the automatic overflow lint (below) finds any spill. Use it as a hard gate in a fix loop: keep rendering until the command exits 0.

### Automatic text-overflow lint

Every render runs a geometric overflow check and prints to stderr any **bound** text (text with a `containerId`) whose measured size spills outside its container box:

```
⚠ TEXT OVERFLOW: 1 text element(s) spill outside their container:
  - 'instanceSystem  vec4 floats 8..11 · flipX, …'  (+62px horizontal)
