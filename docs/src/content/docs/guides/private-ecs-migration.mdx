...
`three-flatland` no longer requires Koota. Remove `koota` from your application when it was installed
only to satisfy the former peer dependency, and keep it when your game has its own Koota world or
systems.

The [`koota-peer-dependency-removal` codemod](https://github.com/thejustinwalsh/three-flatland/blob/main/packages/three-flatland/codemods/koota-peer-dependency-removal.md)
makes that decision per manifest. After installing, it ships at
`node_modules/three-flatland/codemods/koota-peer-dependency-removal.md`.

Application state can still use any ECS. Store a `Sprite2D` reference or an application-owned ID in
that world instead of a three-flatland entity handle.
...
Effect vector getters return read-only tuple snapshots. Assign the complete tuple to publish through
the effect's live backing storage. Mutating a returned snapshot, such as `offset.amount[0] = 4`, does
not update the attached effect.

The [`effect-vector-whole-tuple` codemod](https://github.com/thejustinwalsh/three-flatland/blob/main/packages/three-flatland/codemods/effect-vector-whole-tuple.md)
rewrites component writes and flags dynamic or aliased mutations for review.
