...
import animationPipeline from '../../../assets/diagrams/animation-pipeline.svg?raw'
...
- How to advance a large `SpriteGroup` once while keeping time in your application
...
<Diagram
  caption="The animation data path — atlas frames are grouped into named clips, then play() picks the active clip and update(deltaMs) advances its frames over time."
  src={animationPipeline}
/>
...
<Tabs syncKey="framework">
  ...
  <TabItem label="React" icon="simple-icons:react">
    ...
    import { AnimatedSprite2D, SpriteSheetLoader, SortLayers } from 'three-flatland/react';
...
### Advance a SpriteGroup once

When many animated sprites already belong to one `SpriteGroup`, advance the group instead of keeping
...
<Tabs syncKey="framework">
  <TabItem label="Three.js" icon="simple-icons:threedotjs">
    ...
    spriteGroup.addSprites(...animatedSprites)

    function animate() {
      const deltaMs = clock.getDelta() * 1000
      spriteGroup.advanceAnimations(deltaMs)
      renderer.render(scene, camera)
    }
    ```
  ...
  <TabItem label="React" icon="simple-icons:react">
    ...
    import { AnimatedSprite2D, SpriteGroup } from 'three-flatland/react'

    extend({ SpriteGroup, AnimatedSprite2D })

    function AnimatedCrowd() {
      const groupRef = useRef<SpriteGroup>(null)

      useFrame((_, delta) => {
        groupRef.current?.advanceAnimations(delta * 1000)
      })

      return (
        <spriteGroup ref={groupRef}>
          {/* AnimatedSprite2D children */}
        </spriteGroup>
      )
    }
    ```
...
:::note[Design lineage]
This grouped data-oriented path grew from three-flatland's original
[Koota](https://github.com/pmndrs/koota) integration. Koota's typed traits, structure-of-arrays
storage, queries, and systems made the renderer specialization possible. Koota remains the
recommended general-purpose ECS for application and gameplay state.
:::

## Next steps

- [Animation example](../../examples/animation/) — a spritesheet-driven `AnimatedSprite2D` in a runnable scene.
- [`AnimatedSprite2D` API reference](/three-flatland/api/three-flatland/src/classes/animatedsprite2d/) — the full animation-set and playback surface.
- [Sprites](../sprites/) for the underlying `Sprite2D`, and [Loaders](../loaders/) for loading spritesheets.
