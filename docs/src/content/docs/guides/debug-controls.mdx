...
:::note[Alpha package]
The examples in this repo consume `@three-flatland/devtools` via workspace linking today. An npm release is planned so third-party projects can install it directly. Until then, you can clone the package into your own monorepo or copy the helpers out of `packages/devtools/src/`.
:::
...
- `@three-flatland/devtools` — vanilla Three.js: `createPane`, `wireSceneStats`, `addStatsGraph`, `applyTheme`, `FLATLAND_THEME`
- `@three-flatland/devtools/react` — React hooks: `usePane`, `usePaneFolder`, `usePaneInput`, `usePaneButton`, `useStatsMonitor`, `useFpsGraph`
...
The React entry point (`@three-flatland/devtools/react`) mirrors the vanilla API as hooks that work inside an R3F `<Canvas>`. Create the pane with `usePane()`, then wire stats to the R3F frame loop with `useStatsMonitor(stats)`:
