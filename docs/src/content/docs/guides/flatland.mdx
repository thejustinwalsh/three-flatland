...
flatland.add(sprite1, sprite2, light1);
...
For simpler cases, use `addEffect()` to build a post-processing chain without manual `PostProcessing` setup. Flatland auto-initializes post-processing when the first effect is added:

```typescript
import { crtComplete, vignette } from '@three-flatland/core';

// Add effects — post-processing auto-initialized on first render
flatland
  .addEffect((input, uv) => crtComplete(input, uv, { curvature: 0.15 }))
  .addEffect((input, uv) => vignette(input, uv, { offset: 0.5 }));

// Remove a specific effect
flatland.removeEffect(myEffect);

// Clear all effects (disables auto post-processing)
flatland.clearEffects();
```

Each `EffectFn` receives the current color node and UV coordinates, and returns a transformed color node. Effects are chained in order.
...
| `lighting` | `LightingSystem` | Fixed-slot lighting system |
| `globals` | `GlobalUniforms` | Shared global uniforms (time, tint, viewport) |
| `lights` | `readonly Light2D[]` | Collected Light2D instances |
| `effects` | `readonly EffectFn[]` | Current effect chain |
| `postProcessing` | `PostProcessing \| null` | Post-processing instance |
| `passNode` | `TSLNode` | Scene pass node for composing effects |
| `outputNode` | `TSLNode` | Output node for post-processing |
...
| Method | Description |
|--------|-------------|
| `add(...children)` | Add Sprite2D, Light2D, or other objects |
| `remove(...children)` | Remove objects |
| `clear()` | Remove all sprites and lights |
| `render(renderer)` | Render the scene (updates batches, syncs lights, renders) |
| `resize(width, height)` | Update aspect ratio and camera |
| `createLitColorTransform()` | Create a ColorTransformFn for lit materials |
| `addEffect(fn)` | Add an effect to the post-processing chain |
| `removeEffect(fn)` | Remove an effect from the chain |
| `clearEffects()` | Remove all effects |
| `setPostProcessing(pp, pass)` | Manually set post-processing instance |
