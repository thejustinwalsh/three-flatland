...
For assets you've decided are intentionally never baked (procedurally
varied tilesets, throwaway prototypes, asset bundles where shipping a
sidecar isn't worth the bytes), pass `forceRuntime: true` on the loader.
The runtime bake becomes the canonical source and the "no baked sibling"
warning is suppressed — because choosing `forceRuntime` is itself the
answer to "why isn't there a sidecar?" Mirrors `SlugFontLoader.forceRuntime`;
one flag across every baked-asset loader in the codebase. Not a dev-iteration
knob — the default path already handles iteration (probe → bake on miss + warn
pointing at `flatland-bake`).
...
- Use `DefaultLightEffect` — it is the canonical preset; tiled culling scales well across scene sizes. Lighter-weight and global-illumination variants are tracked in a follow-up PR
