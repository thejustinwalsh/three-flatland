...
frame, pass `normals: { disableRuntimeBake: true }` to either loader, or set
`loader.disableRuntimeBake = true` on the instance for R3F `useLoader` consumers.

:::note
The dev-mode warning is suppressed in production (`NODE_ENV === 'production'`),
so leaving the fallback enabled is safe for sites where sidecars are always
present — the bake path stays dormant.
:::
