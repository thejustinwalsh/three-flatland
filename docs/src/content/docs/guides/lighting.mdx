...
If you're iterating on a tileset's normal descriptor and the 404-then-bake
round-trip is noise you want to skip, pass `forceRuntime: true` on the loader
to go straight to the in-memory bake — same single-flag pattern every
baked-asset loader uses (e.g. `SlugFontLoader.forceRuntime`).
