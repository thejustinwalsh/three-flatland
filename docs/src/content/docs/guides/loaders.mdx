...
Run `npx flatland-bake normal <source>` ahead of time to skip the in-memory path in production. Pass `forceRuntime: true` on the loader to skip the sidecar probe and go straight to the in-memory bake during development (useful when iterating on tile metadata — same flag every baked-asset loader in the codebase exposes, including `SlugFontLoader`). For the offline workflow — running `flatland-bake` ahead of time, sidecar staleness warnings, CI integration — see the [Baking guide](../baking/).
...
`forceRuntime: true` is a sibling option (not nested inside `normals`) that skips the sidecar probe and goes straight to the in-memory bake. Useful during development when iterating on tile metadata and you don't care about the 404 noise.

The in-memory bake path is lazy-imported, so its bundle cost is not paid unless the fallback fires. A dev-mode warning (`NODE_ENV !== 'production'`) points at `npx flatland-bake normal` when the sidecar is absent. See the [Runtime bake fallback](../lighting/#runtime-bake-fallback) section for the full story.
...
For sprites whose individual frames want different per-frame tilt/bump (rare — most character sheets stay flat with alpha-derived shading) pass a `NormalSourceDescriptor` instead of `true` and provide your own `regions`. To skip the sidecar probe during iteration:
...
// R3F useLoader
const sheet = useLoader(SpriteSheetLoader, './sprites/knight.json', (l) => {
  l.normals = true
  l.forceRuntime = true
})
```
