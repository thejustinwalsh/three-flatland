...
`forceRuntime: true` is a sibling option (not nested inside `normals`) that opts a specific asset out of the baked-sibling pattern entirely. The runtime bake becomes the canonical source; the probe is skipped and the "no baked sibling" warn is suppressed. Use it for assets you've decided will *always* be runtime-generated — procedurally varied tilesets, throwaway prototypes, asset bundles where shipping a sidecar isn't worth the bytes. Same flag every baked-asset loader exposes (`SlugFontLoader.forceRuntime`). **Not a dev-iteration knob** — the default path (probe → bake on miss + warn) already handles iteration.
...
For sprites whose individual frames want different per-frame tilt/bump (rare — most character sheets stay flat with alpha-derived shading) pass a `NormalSourceDescriptor` instead of `true` and provide your own `regions`. For an atlas you've decided will *never* be baked (procedural variation, throwaway prototype, lean bundle), opt it out of the sibling pattern with `forceRuntime: true`:
