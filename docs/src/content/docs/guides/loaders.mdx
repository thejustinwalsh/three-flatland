...
- `{ disableRuntimeBake: true }` — suppress the in-memory bake; resolves to a flat-default texture when the baked sidecar is missing (useful in production where a missing sidecar should fail visibly rather than silently bake)
- `{ skipBakedProbe: true }` — skip the sidecar probe and force an in-memory bake (useful during development when iterating on tile metadata)
...
For sprites whose individual frames want different per-frame tilt/bump (rare — most character sheets stay flat with alpha-derived shading) pass a `NormalSourceDescriptor` instead of `true` and provide your own `regions`. To disable the in-memory bake fallback for production builds where all sidecars are guaranteed to be present:

```typescript
// Three.js
const sheet = await SpriteSheetLoader.load('./sprites/knight.json', {
  normals: { disableRuntimeBake: true },
})

// R3F useLoader
const sheet = useLoader(SpriteSheetLoader, './sprites/knight.json', (l) => {
  l.normals = true
  l.disableRuntimeBake = true
})
```
