...
import batchingPipeline from '../../../assets/diagrams/batching-pipeline.svg?raw'
...
<Diagram
  caption="The batch pipeline — sprites group by material and layer into instanced batches packed in one interleaved buffer, with a bucketed dirty tracker and branchless effect gating."
  src={batchingPipeline}
/>
...
`expectedSprites` is a constructor-time hint for scenes whose approximate sprite population is known. It reserves the hot CPU-side enrollment storage used by `SpriteGroup`; it does not pre-create GPU batches or limit how many sprites can be added. `Flatland` accepts the same option and forwards it to its internal `SpriteGroup`.

<Tabs syncKey="framework">
  <TabItem label="Three.js" icon="simple-icons:threedotjs">
    ```typescript
    const spriteGroup = new SpriteGroup({ expectedSprites: 16_384 })

    // Flatland accepts the same constructor option.
  ...
  <TabItem label="React" icon="simple-icons:react">
    ...
    function World() {
      const groupArgs = useMemo(
        () => [{ expectedSprites: 16_384 }] satisfies [SpriteGroupOptions],
        [],
      )
...
The hint is advisory, not a hard cap. Enrollment continues automatically past it with bounded storage growth. `clear()` keeps the reservation available for reuse; `dispose()` releases it.
...
```typescript
const spriteGroup = new SpriteGroup({
  maxBatchSize: 16384, // Optional fixed size; default uses 1024 → 4096 → 16384 tiers
  ...
  clipRect: [-160, -90, 320, 180], // Optional local [x, y, width, height]
})
scene.add(spriteGroup)
```

`maxBatchSize` must be a positive integer no greater than 1,048,576. The default tier ladder is tuned for normal
sprite workloads; use a fixed size only when a measured scene benefits from a specific batch capacity.

## Scene-graph hierarchy

Batched sprites keep the transforms and visibility of their authored parents. Put an ordinary three.js `Group` under a `SpriteGroup` when you want to move, rotate, scale, or hide a whole subtree without giving up batching:
...
<Tabs syncKey="framework">
  <TabItem label="Three.js" icon="simple-icons:threedotjs">
    ...
    page.add(title, portrait, nextButton)
    viewport.add(page)
    scene.add(viewport)

    page.position.x = 80
    page.visible = false // the batch and pointer picking both follow
    ```
...
`clipRect` is local to the `SpriteGroup`, so it follows the group's own transform. Nested `SpriteGroup` clip rectangles compose, and clipped pixels are excluded from pointer hits as well as rendering. Scene-graph clipping uses Three.js `ClippingGroup`, so the rendering half requires `WebGPURenderer`; CPU pointer filtering still applies independently.

Use `SpriteGroup.clipRect` for clipping inside a batch. An ordinary `ClippingGroup` inserted between a `SpriteGroup` and its authored sprites is not retained as a batch boundary, so it cannot clip the shared batch mesh or its pointer hits.

Setting a custom `sprite.renderOrder` opts that sprite out of batching so Three.js can preserve its exact render-list position. Prefer `sortLayer` when the sprite should remain in the batch.

:::caution[Hierarchy behavior is now authoritative]
Older releases flattened some batched sprites against the batching root. If your scene added a compensating transform or manually mirrored ancestor visibility to work around that behavior, remove the workaround when upgrading.

`sprite.visible` keeps normal three.js semantics in every rendering mode: it reports the sprite's locally authored value. The effective batch-slot visibility also includes content readiness and every authored ancestor, but those conditions do not overwrite the public property. A child can therefore report `visible === true` while an invisible parent keeps the subtree out of the batch draw, matching `Object3D` behavior.
...
For a static scene with `autoInvalidateTransforms: false`, call `spriteGroup.invalidateTransforms()` after changing a parent transform or parent visibility. Direct `sprite.visible` changes invalidate their batch slot automatically.
...
backgrounds.forEach((s) => (s.sortLayer = SortLayers.BACKGROUND))
...
import bufferDataflow from '../../../assets/diagrams/instance-buffer-dataflow.svg?raw'

<Diagram caption="Per-instance data flow: CPU → GPU → TSL accessors" src={bufferDataflow} />
