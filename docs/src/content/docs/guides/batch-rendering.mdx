...
While a source sprite is represented by a batch, reading `sprite.visible` returns `false` because its own mesh is privately suppressed. Assigning `sprite.visible` still records the authored value and updates the batch. Keep application visibility in your own state when you need to read it back directly.
:::
