...
<Diagram caption="The batch pipeline — sprites group by material and layer into instanced batches packed in one interleaved buffer, with a bucketed dirty tracker and branchless effect gating." src={batchingPipeline} />
...
Property changes on sprites are detected automatically through the ECS. When you update a sprite's tint, alpha, flip, frame, sortLayer, or position, the renderer syncs only the changed data to the GPU on the next frame. No manual invalidation needed.
