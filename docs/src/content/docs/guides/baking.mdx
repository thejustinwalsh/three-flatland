...
- **`forceRuntime: true`** — passed on the loader. Skips the sibling probe entirely and goes straight to the in-memory bake. Useful while iterating on tile metadata when you know the sidecar is stale and don't care to refresh it on every change. Mirrors `SlugFontLoader.forceRuntime` — the same flag every baked-asset loader in the codebase exposes.
...
| **Forced runtime** (`forceRuntime: true`) | Same as runtime, plus no probe round-trip and no warning | Tightening the iteration loop when you know the sidecar is stale |
