...
import skiaRenderModes from '../../../assets/diagrams/skia-render-modes.svg?raw';

**What you'll learn**
- How to initialize Skia, mount a `SkiaCanvas` in texture or overlay mode, and render shapes
- How to load fonts and apply custom paints (gradients, blend modes, blur, drop shadow)
- How to perform programmatic path operations — `union`, `intersect`, `difference`, `xor`

---

:::caution[Experimental]
The `@three-flatland/skia` API isn't settled yet. We're dogfooding it across the examples and folding in [community feedback](https://github.com/thejustinwalsh/three-flatland/issues?q=is%3Aissue+label%3Askia) to stabilize it, so it can shift between releases. Pin an exact version for now.
:::
...
<Tabs syncKey="pkg">
  ...
  <TabItem label="yarn">
    ```bash
    yarn add @three-flatland/skia three
    ```
  </TabItem>
  <TabItem label="bun">
    ```bash
    bun add @three-flatland/skia
    ```
  </TabItem>
  <TabItem label="agents">
    ...
    Install: npm install @three-flatland/skia
    Requires: three ^0.185.1, auto-detects WebGPU or WebGL from renderer
...
Peer dependencies: `three ^0.185.1`. For React: `@react-three/fiber 10.0.0-alpha.3` and `react ^19.2.0`.
...
## SkiaCanvas :badge[Alpha]{variant=caution}

The root node that owns the render target and walks children to draw. Two modes:

<Diagram caption="SkiaCanvas render modes — texture mode draws before the scene and feeds a mesh map; overlay mode composites onto the framebuffer after the scene." src={skiaRenderModes} />
...
## SkiaGroup :badge[Alpha]{variant=caution}
...
`SkiaFontLoader` loads a `SkiaTypeface` — the font data independent of size. Call `.atSize(n)` to get a sized `SkiaFont`.
