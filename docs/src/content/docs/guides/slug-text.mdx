...
import slugFontLoading from '../../../assets/diagrams/slug-font-loading.svg?raw'
...
:::caution[Experimental — API not stable]
`@three-flatland/slug` is **more experimental than the rest of the alpha library**: its public API is not yet stable and can change or be removed without notice between releases. Pin an exact version and expect breakage on upgrade. Active feature work is tracked in [#37 — Vector Graphics](https://github.com/thejustinwalsh/three-flatland/issues/37) and [#38 — Rich Text](https://github.com/thejustinwalsh/three-flatland/issues/38).
:::
...
<Diagram caption="SlugFontLoader resolution — a baked .slug.glb loads directly and skips opentype.js; a miss falls back to fetching and parsing the .ttf at runtime." src={slugFontLoading} />
