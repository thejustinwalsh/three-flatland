...
Use `TextureLoader` from `three-flatland` (or `three-flatland/react`) to load textures with automatic preset configuration. For spritesheets, use `SpriteSheetLoader`. See the [Loaders guide](../guides/loaders) for details.
:::
...
:::tip[Flatland]
For a complete 2D pipeline with camera, post-processing, and global uniforms, use [Flatland](../guides/flatland) instead of adding a SpriteGroup to your scene manually.
:::
