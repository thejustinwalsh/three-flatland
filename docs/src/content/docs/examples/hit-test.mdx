...
:::tip[Batched sprites use the same API]
Sprites managed by `SpriteGroup` or `Flatland` stay individually interactive. The batch performs the broad-phase lookup and reports the original `Sprite2D` as `hit.object`; ancestor transforms, visibility, and `SpriteGroup.clipRect` are honored by both rendering and picking.
:::
