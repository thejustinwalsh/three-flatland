...
`SlugFontLoader.load()` is the single entry point. It tries baked data (`.slug.json` + `.slug.bin`) first and falls back to runtime parsing of TTF/OTF/WOFF — opentype.js is dynamic-imported only on the runtime path, so baked-only bundles ship without it.
