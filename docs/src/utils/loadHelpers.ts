...
export function loadDirectoryFiles(dir: string, prefix: string, files: Record<string, string>): void {
...
export function loadPublicFiles(dir: string, prefix: string, files: Record<string, string>): void {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    const fullPath = path.join(dir, entry.name)
    const relativePath = `${prefix}/${entry.name}`
