...
async function test(name, hideHero) {
  ...
  console.log(`[${name.padEnd(32)}] frames=${r.frames}/300 p50=${r.p50.toFixed(0)}ms p95=${r.p95.toFixed(0)}ms stutter=${r.stutterPct}%`)
  await ctx.close()
}
try {
  console.log('Landing page in Webkit. 3 runs each.\n')
  console.log('--- HeroShader ENABLED ---')
  for (let i=0;i<3;i++) await test(`run ${i+1} hero on`, false)
  console.log('--- HeroShader HIDDEN ---')
  for (let i=0;i<3;i++) await test(`run ${i+1} hero off`, true)
