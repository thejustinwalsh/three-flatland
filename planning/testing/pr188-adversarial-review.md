...
1. **P0 — CI still launches the real audio device path after PulseAudio was removed.**
   ...
   [audio-play.spec.ts:32](tools/vscode/e2e/specs/audio-play.spec.ts:32), [audio-play.spec.ts:59](tools/vscode/e2e/specs/audio-play.spec.ts:59), and [audio-play.spec.ts:91](tools/vscode/e2e/specs/audio-play.spec.ts:91) all execute `playParams`. That spawns the production sidecar, whose import of `zzfx` constructs `new AudioContext` at module load ([sidecar.ts:15](tools/audio-play/src/sidecar.ts:15), [ZzFX.js:62](node_modules/.pnpm/zzfx@1.3.2/node_modules/zzfx/ZzFX.js:62)). `node-web-audio-api` immediately calls the native `NapiAudioContext` constructor and rethrows native failures ([AudioContext.js:63](node_modules/.pnpm/node-web-audio-api@2.0.0/node_modules/node-web-audio-api/js/AudioContext.js:63)).
...
2. **P0 — the offline probe has an exit/stdio race and can intermittently report “no verdict.”**
   ...
   The child calls `console.log(...)` and immediately `process.exit(0)` ([offlineRenderProbe.mjs:56](tools/vscode/e2e/host-bridge/offlineRenderProbe.mjs:56)). Node explicitly does not guarantee pending pipe writes are flushed by `process.exit()`.

   The parent compounds that by inspecting stdout on the child’s `exit` event ([audio-render-gate.spec.ts:67](tools/vscode/e2e/specs/audio-render-gate.spec.ts:67)); `exit` may precede the child stdio streams’ `close` event and final `data` delivery.
...
3. **P0 — the rewritten audio-lens suite still contains exactly the forbidden deadline polling.**
   ...
   `fetchSettledLenses` polls every 150 ms until a 15-second wall-clock deadline ([zzfx-audio-lenses.spec.ts:155](tools/vscode/e2e/specs/zzfx-audio-lenses.spec.ts:155)). `pollLensAt` repeats the same pattern with a 5-second deadline ([zzfx-audio-lenses.spec.ts:204](tools/vscode/e2e/specs/zzfx-audio-lenses.spec.ts:204)).
...
4. **P0 — the BroadcastChannel echo-guard barrier is not a valid barrier.**
   ...
   The filter barrier at [bus-websocket.test.ts:414](packages/three-flatland/src/debug/bus-websocket.test.ts:414) is different: all three messages originate from the same `local` object, so FIFO is defensible there.

5. **P1 — “dispatch without throwing” is false coverage for fire-and-forget sidecar commands.**

   The rewritten specs claim full command completion at [zzfx-audio-lenses.spec.ts:181](tools/vscode/e2e/specs/zzfx-audio-lenses.spec.ts:181) and [zzfx-synth-lenses.spec.ts:118](tools/vscode/e2e/specs/zzfx-synth-lenses.spec.ts:118). But `playSong` and `playWadSynth` merely write to the child and return ([register.ts:424](tools/vscode/extension/tools/audio/register.ts:424), [register.ts:467](tools/vscode/extension/tools/audio/register.ts:467)). They do not await an Ack.
...
6. **P1 — unique source-editor binding coverage was deleted with no replacement.**
   ...
   Production behavior lives in three VS Code event handlers at [register.ts:241](tools/vscode/extension/tools/audio/register.ts:241): active-editor change, tab-group change, and document close. Neither `commandHandler.test.ts` nor `player.test.ts` exercises VS Code tabs, `ActivePlayback.sourceUri`, or these listeners.

   The removed “switching/closing the source document stops playback” e2e was the only test covering this wiring. It was load-bearing—comments at [register.ts:227](tools/vscode/extension/tools/audio/register.ts:227) explicitly say the tab event is required because `onDidCloseTextDocument` is insufficient.
...
7. **P1 — the shared window still leaks timer-driven and process state across tests.**
   ...
   The fixture only closes tabs, clears workspace configuration, and recopies files ([fixtures.ts:151](tools/vscode/e2e/fixtures.ts:151)). The extension host, sidecars, active-playback singleton, resolver caches, global storage, and audio context all survive because the same window is reused across every file ([fixtures.ts:219](tools/vscode/e2e/fixtures.ts:219)).

   Worse, the fixture deliberately sets the sidecar’s idle release to five seconds ([fixtures.ts:90](tools/vscode/e2e/fixtures.ts:90)). Whether a later test uses the existing context or exercises reacquisition therefore depends on how long preceding tests took. That is direct timer/order dependence.
...
8. **P1 — the Tone tests still wait on fixed production backoff timers.**
   ...
   [zzfx-synth-lenses.spec.ts:443](tools/vscode/e2e/specs/zzfx-synth-lenses.spec.ts:443) explicitly waits through the retry schedule. That schedule is four fixed sleeps at [toneColdStartRetry.ts:56](tools/vscode/extension/tools/audio/toneColdStartRetry.ts:56) and [toneColdStartRetry.ts:87](tools/vscode/extension/tools/audio/toneColdStartRetry.ts:87).
...
11. **P2 — the relay rewrite retains unseeded randomness unnecessarily.**
   ...
   WebSocket handshake keys still use `Math.random()` at [relay.test.ts:163](packages/devtools/src/relay.test.ts:163) and [relay.test.ts:261](packages/devtools/src/relay.test.ts:261). It is unlikely to fail, but it directly violates the stated rule and adds no coverage. Use a fixed valid 16-byte RFC example key.

## CI assessment

The explicit masking layers are gone: Playwright retries are zero ([playwright.config.ts:33](tools/vscode/e2e/playwright.config.ts:33)), and the workflow invokes the test command once ([vscode-e2e.yml:98](.github/workflows/vscode-e2e.yml:98)). I found no remaining `continue-on-error` or retry wrapper on this job.

However, CI is not reproducible across time: it uses `ubuntu-latest`, `node-version: lts/*`, a stable Rust toolchain, and VS Code `stable` ([vscode-e2e.yml:9](.github/workflows/vscode-e2e.yml:9), [vscode-e2e.yml:20](.github/workflows/vscode-e2e.yml:20), [fixtures.ts:194](tools/vscode/e2e/fixtures.ts:194)). Pinning VS Code is particularly important for an Electron-native regression gate.
