...
- `usePaneRadioGrid` hook (React subpath) — inline button-bar selector backed by Tweakpane Essentials `radiogrid` blade; deferred disposal and synchronous creation match the existing `usePaneButton`/`usePaneInput` pattern
- `PaneInputOptions` extended with `readonly` and `format` fields for readonly monitor inputs with custom formatters

### Bug fixes

- Fixed `z-index` application: pane wrapper (`z-index: 1000`) now correctly targets the `.tp-dfwv` body-sibling element instead of the inner `pane.element`
- Fixed checkbox hit target: `.tp-ckbv_i` stretched to full box size so clicks land directly on the input without relying on flaky `<label>` forwarding
- Checkbox theme: box surface color now matches other controls (`rgba(28,40,77,0.6)`) with hover/focus/active parity; check stroke turns accent pink on `:checked`

Adds `usePaneRadioGrid` hook and `readonly`/`format` input options; fixes checkbox hit target reliability and z-index stacking for the pane wrapper.
