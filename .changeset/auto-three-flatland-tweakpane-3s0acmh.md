...
- `usePaneRadioGrid` hook (react subpath) — inline button-bar selector backed by the tweakpane-essentials `radiogrid` blade; deferred disposal and synchronous creation mirror the existing `usePaneButton`/`usePaneInput` pattern
...
- Checkbox hit target: `.tp-ckbv_i` now stretches to the full `var(--cnt-usz)` box size; clicks land on the input directly without requiring label-forwarding, which was flaky under certain pointer-event/z-index combinations
- Checkbox theme: box surface updated to `rgba(28,40,77,0.6)` with hover/focus/active parity; check stroke turns accent pink on `:checked`; previously the invisible hit target and default box blending made checkboxes effectively undiscoverable
- `z-index: 1000` now applied to the `.tp-dfwv` body-sibling wrapper element instead of the inner pane root; previously the z-index had no effect on stacking against other overlays
- `useWindowSize` now tracks `{ w, h, dpr }` and subscribes to a `(resolution: Ndppx)` media query so monitor swaps trigger a canvas resize; previously a DPR change without a dimension change left the compare canvas at the old pixel ratio
- Fullscreen state: added a `document.fullscreenchange` listener that re-measures immediately and again in the next `requestAnimationFrame`, catching post-transition layout settles that the `resize` event alone misses

`@three-flatland/tweakpane` gains a radio-grid hook for scene toggles, reliable checkbox interaction, and correct pane stacking across fullscreen and monitor-swap transitions.
