...
- `usePaneRadioGrid(pane, options)` — React hook (react subpath) backed by `@tweakpane/plugin-essentials` radiogrid blade; inline button-bar selector with active-state affordance, mirrors the `usePaneButton` / `usePaneInput` disposal pattern
- `PaneInputOptions` extended with `readonly` and `format` — allows readonly monitors with custom value formatters via `usePaneInput`

## Fixes

- `createPane`: `z-index` now applied to the `.tp-dfwv` body-sibling wrapper instead of the inner pane root — setting it on the inner element had no effect on stacking against overlays
- Checkbox CSS: `.tp-ckbv_i` stretched to full `var(--cnt-usz)` box so clicks land directly on the input without relying on `<label>` forwarding — fixes multiple-click-to-toggle in some pointer-events / z-index combinations
...
Adds the `usePaneRadioGrid` hook, readonly monitor support, and fixes checkbox interaction and stacking reliability.
