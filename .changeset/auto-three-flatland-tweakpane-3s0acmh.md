...
- `usePaneRadioGrid` hook (react subpath) — inline button-bar selector backed by Tweakpane essentials `radiogrid` blade; active-state affordance reads better than a dropdown for scene/mode toggles; deferred disposal and synchronous creation match the existing `usePaneButton`/`usePaneInput` pattern
...
- Checkbox hit target stretched to full control box size via `width/height: var(--cnt-usz)` on `.tp-ckbv_i`; clicks now land directly on the input without relying on flaky `<label>` forwarding
- `z-index: 1000` now applied to the `.tp-dfwv` body-sibling wrapper instead of the inner pane root element, so the pane stacks correctly above other overlays

## Theme

- Checkbox box surface matches other controls (`rgba(28,40,77,0.6)`) with hover/focus/active parity
- Check stroke turns accent pink on `:checked`; previous default blended invisibly with the container

Minor release adding `usePaneRadioGrid`, readonly/format monitor support, and checkbox hit-target and theme fixes.
