...
- `usePaneRadioGrid` hook (react subpath) — inline button-bar selector backed by essentials' `radiogrid` blade; deferred disposal and synchronous creation mirror `usePaneButton`/`usePaneInput`
- `PaneInputOptions.readonly` + `PaneInputOptions.format` — create readonly monitors with custom formatters via the existing `usePaneInput` hook

### Bug Fixes

- z-index fix: apply `z-index: 1000` to the `.tp-dfwv` body-sibling wrapper instead of the inner pane root — pane now correctly stacks above overlays
- Checkbox hit target: `.tp-ckbv_i` stretched to `var(--cnt-usz)` on both axes — clicks land on the input directly without relying on flaky `<label>` forwarding
...
Adds `usePaneRadioGrid`, readonly monitor support, and fixes z-index stacking and checkbox hit-target/theming issues.
