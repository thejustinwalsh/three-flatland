...
- **`usePaneRadioGrid` hook** (react subpath): inline radio button-bar selector backed by the `@tweakpane/plugin-essentials` radiogrid blade; deferred disposal and synchronous creation mirror the existing `usePaneButton` / `usePaneInput` pattern
- **`PaneInputOptions.readonly` + `format`**: allows React hook users to declare read-only monitors with custom value formatters

## Bug fixes

- `z-index` is now applied to the `.tp-dfwv` wrapper element Tweakpane creates when no container is provided; previously applied to the inner pane root, which had no effect on stacking against other overlays
- Checkbox hit target fixed: `.tp-ckbv_i` stretched to full `var(--cnt-usz)` box so clicks land directly on the input without relying on flaky `<label>` click-forwarding
...
Adds an inline radio-grid hook, read-only monitor support, and fixes Tweakpane's stacking behavior and checkbox usability.
