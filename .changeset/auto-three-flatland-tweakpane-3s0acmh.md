...
- `usePaneRadioGrid(pane, options)` hook (react subpath) — inline button-bar selector backed by Tweakpane Essentials `radiogrid` blade; deferred disposal + synchronous creation match existing `usePaneButton` / `usePaneInput` pattern
- `PaneInputOptions` extended with `readonly` and `format` fields — enables readonly monitor bindings with custom value formatters

**Bug fixes**

- `createPane` now applies `z-index: 1000` to the `.tp-dfwv` outer wrapper (the body-sibling stacking context) instead of the inner pane root; setting it on the inner element had no effect on overlay stacking
- Checkbox hit target stretched to fill the full 20×20 control box via `width/height: var(--cnt-usz)` on `.tp-ckbv_i`; eliminates multi-click failures caused by flaky `<label>` → `<input>` click forwarding under certain pointer-events / z-index combinations

**Theme**

- Checkbox box surface styled to `rgba(28, 40, 77, 0.6)` with hover / focus / active parity matching other controls; check stroke switches to accent pink on `:checked`; previous default blended invisibly with the container background

**Examples**

- `slug-text` examples (React + Three) migrated from Web Awesome (`@awesome.me/webawesome`) to `@three-flatland/tweakpane`; all controls now use `createPane`, `usePaneFolder`, `usePaneInput`, and `useStatsMonitor`
- Computing spinner relocated from top-right to top-left so it no longer conflicts with the Tweakpane panel corner
- Overlay z-indexes set to 1–4 so the Tweakpane wrapper (z-index 1000) consistently sits above all other overlays

This release replaces the Web Awesome dependency in examples, fixes checkbox usability, and adds the `usePaneRadioGrid` hook and readonly monitor support to the React bindings.
