...
- `usePaneRadioGrid(pane, cells, options)` hook (react subpath) — inline button-bar selector backed by essentials' `radiogrid` blade; deferred disposal and synchronous creation match the existing `usePaneButton`/`usePaneInput` pattern
- `PaneInputOptions.readonly` + `format` — create readonly monitors with custom formatters via the React hook surface

## Bug Fixes

- Checkbox hit target stretched to full 20×20 box via explicit `width/height: var(--cnt-usz)` on `.tp-ckbv_i`; eliminates flaky label-forwarding clicks in certain pointer-events/z-index combinations
- `createPane` z-index now applied to the `.tp-dfwv` body-sibling wrapper instead of the inner pane root; setting it on the inner element had no effect on stacking context

## Theme

- Checkbox box surface changed to `rgba(28,40,77,0.6)` with hover/focus/active parity matching other controls; previously blended with the container, making the hit target visually invisible
- Check stroke turns accent pink on `:checked`

## Examples

- Both React and Three `slug-text` examples migrated from Web Awesome (`@awesome.me/webawesome`) to `@three-flatland/tweakpane`; `wa-*` selectors, `useWrappingGroup`/`setupWrappingGroup` helpers, and status `div` removed
- Tweakpane pane z-index 1000 sits clearly above all overlays (z-index 1–4); computing spinner moved top-right → top-left to avoid tweakpane corner conflict
- DPR re-sync on R3F canvas after monitor swap / fullscreen via new `<DprSync>` component; all three resize sources (resize event, `(resolution: Ndppx)` media query, `fullscreenchange`) flow through to both canvases simultaneously

Adds `usePaneRadioGrid`, readonly monitor support, and resolves DPR desync, fullscreen state, and checkbox reliability issues.
