...
- `SlugFont.measureParagraph(text, fontSize, { maxWidth?, lineHeight? })` → `ParagraphMetrics` — multi-line block dimensions using the same wrap policy as `SlugText`
- `SlugFont.wrapText(text, fontSize, maxWidth?)` → `string[]` — returns line-broken text matching `SlugText` output exactly
- `SlugFont.hasCharCode(codepoint)` — cheap codepoint coverage check via font cmap
- `SlugFontStack(fonts)` — ordered font chain with per-codepoint fallback; `resolveCodepoint`, `resolveText`, `wrapText`, `emitDecorations`
- `SlugStackText` — multi-font `Group` with one `InstancedMesh` per font; shares shaping pipeline with `SlugText`; supports `styles`, `outline`, `setOpacity`
- `SlugText.outline` — opt-in stroke rendered behind fill via a child `InstancedMesh` + `SlugStrokeMaterial`; `setOutlineWidth` / `setOutlineColor` update uniforms with zero rebuild
- `SlugText.setOpacity(value)` — adjusts fill material opacity uniform directly (enables outline-only mode)
- `SlugText.styles` — `StyleSpan[]` for underline and strikethrough decorations, rendered in the same draw call as glyph instances
- `SlugStrokeMaterial` — analytic distance-to-curve stroke material; exported from package root with `SlugOutlineOptions`
- `SlugFont.emitDecorations` / `pipeline/decorations.emitDecorations` — post-shaping decoration rect emitter
...
- `--output` / `-o` — custom output base path for `.slug.{json,bin}` files
- `--stroke-widths` / `--stroke-join` / `--stroke-cap` / `--miter-limit` — bake pre-computed stroke contours into the font asset; stroke glyphs render via the existing fill shader at zero extra cost
- Baked format gains optional `strokeSets` field; absent for fonts baked without stroke flags, so old assets load unchanged

**Stroke pipeline (Phase 5)**

- `strokeOffsetter` — quadratic-Bézier stroke pipeline: adaptive subdivision, per-segment Tiller-Hanson offset, bevel/miter/round join insertion, flat/square/round/triangle cap insertion, outer+inner contour stitching
- `bakeStrokeForGlyph(source, options)` — converts a filled `SlugGlyphData` into a stroked one; used by CLI bake pass and future runtime fallback
...
- `curveTexture` format: `RGBA32F` → `RGBA16F` (half-float); halves texture bandwidth
- `bandTexture` format: `RGBA32F` → `RG32F`; eliminates wasted channels
- `MAX_CURVES_PER_BAND`: 64 → 40; matches actual Inter corpus p999 (38), reduces shader register pressure
- Band count: 8 → 16; halves expected curves per band in the fragment hot loop
- Shader skips solve + coverage work for curves that don't cross the ray (~30% of curves in a band)
- Stroke fragment shader: Newton seeds reduced from 3×3 to 1 seed with 3 iterations + 2 endpoint candidates; halves WGSL size and first-draw compile stall
- Stroke quad expansion moved axis-aligned into `SlugStrokeMaterial` vertex shader; fixes clipped outer corners at glyph bbox extents

**Bug fixes**

- `SlugText._setFont` no longer sets `visible=true` before the first `_rebuild`; prevents WebGPU pipeline errors on R3F's first render pass
- `SlugStackText.dispose()` now tears down outline meshes and per-font fill meshes in the correct order (no double-dispose, no GPU leaks on scene toggle)
- Runtime shapers pass `{ features: [] }` to `stringToGlyphs`; prevents `liga`/`rlig` from collapsing word-boundary whitespace in wrapped text
- `parseFont` emits advance-only entries for cmap'd glyphs with no outline (space, tab, zero-width controls); advances now match baked path
- Kerning extractor filters to source glyph IDs only; fixes `this.font._push is not a function` when stroke IDs are present

**Removals**

- `SlugFontLoader.clearCache` removed (static cache is already keyed per URL)
- `BAKED_VERSION` machinery removed from `SlugFontLoader` (package not yet released; no migration story)
- `slugDilate`'s `strokeHalfWidth` parameter removed; stroke expansion is now handled in `SlugStrokeMaterial`

**Examples**

- `examples/vanilla/slug-text` moved to `examples/three/slug-text`; package renamed `example-three-slug-text`
- Both Three.js and React examples achieve 1:1 feature parity throughout: Canvas2D compare overlay (onion / diff / split / off modes), draggable split handle, measure overlay with click-to-select, Styles folder, Outline folder, [Lorem | Icons] scene toggle with FA-Solid icon demo
- Icons mode: 12-glyph FA-Solid subset baked with `slug-bake` (~71 KB bin); `@font-face` with `font-weight: normal` for Canvas2D fallback
- Compare overlay uses `stack.wrapText` in icons mode so line breaks agree with `SlugStackText`
- React: `<DprSync>` component keeps R3F canvas pixel ratio in sync after monitor swaps and fullscreen transitions

**BREAKING CHANGES**

- **BAKED_VERSION 2 → 3**: `curveTexture` is now `RGBA16F`, `bandTexture` is `RG32F`, `MAX_CURVES_PER_BAND` is 40. All existing `.slug.bin`/`.slug.json` files must be re-baked with the current `slug-bake` CLI.
- **BAKED_VERSION 3 → 4**: decoration metrics (`underlinePosition`, `underlineThickness`, `strikethroughPosition`, `strikethroughThickness`, subscript/superscript fields) added to `BakedJSON.metrics`. All `.slug.bin`/`.slug.json` files must be re-baked.
- `slugDilate` no longer accepts a `strokeHalfWidth` parameter; stroke quad expansion is handled inside `SlugStrokeMaterial`.
- `SlugFontLoader.clearCache` is removed.

This release ships the complete Phases 1–5 of the Slug text rendering library: analytic WebGPU text with baked font support, measurement APIs, text decorations, multi-font fallback stacks, runtime-uniform outlines, and a full quadratic-Bézier stroke offsetter pipeline for baking precise stroke geometry.
