...
- `SlugFont.measureParagraph(text, fontSize, { maxWidth?, lineHeight? })` — multi-line block metrics; height matches rendered output
- `SlugFont.wrapText(text, fontSize, maxWidth?)` — line-wrap dispatch for baked and runtime paths
- `SlugFont.hasCharCode(c)` — codepoint coverage check via font cmap
- `SlugFontStack(fonts)` — per-codepoint font fallback chain; `resolveCodepoint` / `resolveText` for font-index assignment
- `SlugFontStack.wrapText(text, fontSize, maxWidth?)` — multi-font line-wrap; keeps Canvas2D comparison line-for-line with `SlugStackText`
- `SlugStackText` — multi-font renderable extending `Group`; one `InstancedMesh` per contributing font
- `SlugStackText.styles` — underline/strikethrough span support via `SlugFontStack.emitDecorations()`
- `SlugStackText.outline` / `setOutlineWidth()` / `setOutlineColor()` / `setOpacity()` — parity with `SlugText`
- `SlugText.outline` — opt-in child stroke mesh sharing fill geometry; `setOutlineWidth()`, `setOutlineColor()`, `setOpacity()` for zero-rebuild runtime updates
...
- `SlugFont.emitDecorations()` — one rect per contiguous styled run in the same draw call as glyphs
- `SlugFont` exposes decoration metrics: `underlinePosition`, `underlineThickness`, `strikethroughPosition`, `strikethroughThickness`

## CLI

- `slug-bake --output / -o` — custom output base path
- `slug-bake --stroke-widths`, `--stroke-join`, `--stroke-cap`, `--miter-limit` — bake pre-computed stroke glyph variants into `.slug.{json,bin}`; stroke glyphs render through the existing fill shader at 1× fill cost

## Stroke Pipeline

- Analytic `distanceToQuadBezier` primitive — pure-JS reference + TSL port; Newton refinement with clamped parameter
- `slugStroke` fragment shader — distance-to-curve coverage with bevel-via-min joins; sub-pixel hairlines widen to ≥1px
- `SlugStrokeMaterial` — parallels `SlugMaterial`; runtime-uniform `color`, `opacity`, `strokeHalfWidth`
- Full quadratic-Bezier stroke offsetter (`strokeOffsetter`): adaptive subdivision, per-segment Tiller-Hanson offset, bevel/miter/round join insertion, flat/square/triangle/round cap insertion, contour stitching into closed fill-ready contours
- `bakeStrokeForGlyph(source, options)` — bridges offsetter to CLI bake pass and future runtime fallback; preserves source `glyphId`/`advanceWidth`/`lsb`
- `BakedJSON.strokeSets` — optional metadata field for pre-baked stroke variants; absent for fonts baked without stroke flags (backward-compatible)
- `SlugFont.getStrokeGlyph(sourceId, width, join, cap, miterLimit?)` — look up pre-baked stroke glyph data

## Performance

- `curveTexture` → RGBA16F; `bandTexture` → RG32F — ~50% texture bandwidth reduction
- `MAX_CURVES_PER_BAND` 64 → 40; `bandCount` 8 → 16 — fewer curves per fragment, lower register pressure
- Shader skips non-crossing curves (wrap `rootCode > 0` branch) — ~30% of curves culled early
- Stroke shader compile cost halved: single Newton seed (t=0.5) + endpoints; WGSL size roughly halved, per-fragment cost drops ~⅔

## Bug Fixes

- Stroke quad expansion fixed to axis-aligned per-axis growth; previous diagonal-normal expansion clipped outer ring square at glyph extents
- `SlugText._setFont` defers `visible = true` until after `_rebuild`; prevents blank WebGPU canvas on first R3F render
- Runtime shapers pass `{ features: [] }` to opentype.js `stringToGlyphs`; suppresses `liga`/`rlig` token deletion that caused whitespace collapse at wrap points
- `SlugStackText.dispose()` tears down outline meshes and materials before disposing shared geometry; eliminates GPU leak on scene toggle
- `SlugFontLoader` BAKED_VERSION machinery removed; no pre-release migration surface to maintain

## Examples

- React + Three examples have 1:1 parity throughout; both use `examples/react/slug-text` and `examples/three/slug-text`
- Scene toggle: `[Lorem | Icons]` radio — Lorem: single `SlugText`; Icons: `SlugStackText` with [Inter, FA-Solid] stack + 12-icon FA-Solid subset baked via `slug-bake`
- Canvas2D compare overlay with onion/split/diff modes; draggable split handle; `stack.wrapText` keeps line breaks in sync with Slug output
- Click-to-select line measure overlay (cyan ink bounds, dashed yellow font envelope); paragraph monitors live-update
- Styles folder: underline/strike presets via `StyleSpan` API
- Outline folder: Fill/Outline/Both radio; live width slider and color picker
- Compare mode gains `Off` option to hide overlay entirely
- Migrated from Web Awesome controls to `@three-flatland/tweakpane`
- `renderer.antialias: false` — analytic per-fragment coverage makes MSAA redundant

## BREAKING CHANGES

- **BAKED_VERSION 2 → 3**: `curveTexture` format changed to RGBA16F, `bandTexture` to RG32F. All `.slug.bin`/`.slug.json` files must be re-baked with the updated `slug-bake` CLI.
- **BAKED_VERSION 3 → 4**: `BakedJSON.metrics` gains decoration fields (`underlinePosition`, etc.). All `.slug.bin`/`.slug.json` files must be re-baked.

Adds a full analytic text-rendering pipeline: GPU stroke outlines, font-stack fallback, text measurement, underline/strikethrough decorations, and a quadratic-Bezier stroke bake path — all with 1:1 React/Three example coverage.
