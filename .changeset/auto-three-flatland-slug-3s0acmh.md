...
**Text measurement**
- `SlugFont.measureText(text, fontSize)` → `TextMetrics` — CanvasRenderingContext2D-compatible single-line measurement
- `SlugFont.measureParagraph(text, fontSize, { maxWidth?, lineHeight? })` → `ParagraphMetrics` — multi-line measurement respecting SlugText line-height default
- `SlugFont.wrapText(text, fontSize, maxWidth?)` → `string[]` — word-wrap matching shaping output exactly
- `SlugFont.hasCharCode(codepoint)` — cheap cmap coverage check

**Text decorations**
- `StyleSpan { start, end, underline?, strike? }` — new type for underline/strikethrough ranges
- `SlugText` now accepts `styles?: StyleSpan[]` (constructor + runtime setter)
- `SlugFont.emitDecorations()` / `pipeline/decorations.ts` — post-shaping decoration rect emission in the same draw call via rect-sentinel instances

**Outline / stroke**
- `SlugText.outline` — opt-in stroke rendered as a child `InstancedMesh` behind the fill; shares `instanceMatrix`, zero-copy
- `SlugText.setOutlineWidth(w)` / `setOutlineColor(c)` — runtime-uniform setters, no rebuild
- `SlugText.setOpacity(value)` — fade fill independently (enables outline-only mode)
- `SlugOutlineOptions` exported from package root; `color` accepts `number | string | Color`
- `SlugStrokeMaterial` — TSL stroke NodeMaterial with analytic `distanceToQuadBezier`, bevel-via-min joins, crispness gate (sub-pixel strokes widen to 1px)
...
- `SlugFontStack.emitDecorations()` — decoration emission across mixed-font runs
- `SlugStackText` — `THREE.Group` with one `InstancedMesh` per font; renders mixed-font text in one pass per font
- `SlugStackText.styles`, `.outline`, `.setOpacity()` — parity with `SlugText`

**Stroke bake pipeline (Phase 5)**
- `strokeOffsetter` — quadratic-Bezier stroke offsetter: adaptive subdivision, Tiller-Hanson per-segment offset, miter/bevel/round join insertion, flat/square/triangle/round cap insertion, full contour stitching
- `bakeStrokeForGlyph(source, options)` — build pre-baked stroke `SlugGlyphData` from a source glyph
- `SlugFont.getStrokeGlyph(sourceId, width, join, cap, miterLimit?)` — look up a pre-baked stroke glyph

**CLI**
- `slug-bake` gains `--stroke-widths`, `--stroke-join`, `--stroke-cap`, `--miter-limit` flags for baking stroke sets
- `slug-bake` gains `--output / -o` for custom output path bases

**Infrastructure**
- `buildGpuGlyphData` / `buildGpuGlyphFromCurves` / `buildAdvanceOnlyGlyph` — shared contour-to-GPU pipeline extracted from `fontParser` into `pipeline/buildGpuGlyph.ts`

### Changed

**Baked format** (re-bake required)
- BAKED_VERSION 2→3: curve texture → RGBA16F, band texture → RG32F, bandCount 8→16; `.slug.bin` files ~45% smaller
- BAKED_VERSION 3→4: decoration metrics (underline/strikethrough position + thickness, subscript/superscript scale + offset) added to `BakedJSON.metrics`
- `BakedJSON.strokeSets?` — optional field listing pre-baked stroke sets; absent for fonts baked without stroke flags

**Performance**
- Shader early-exit: non-crossing curves skip `sqrt` + coverage work (`If(rootCode > 0)` guard, ~30% of curves in a band)
- Stroke shader Newton seeds reduced 3→1 with endpoint candidates; halved WGSL size and pipeline compile cost, improved per-fragment GPU time
- Stroke quad expansion fixed to axis-aligned (was diagonal-normal, clipped outer corners at glyph bbox extents)

### Fixed

- `SlugText._setFont` deferred `visible=true` until after first `_rebuild` — prevents zero-binding WebGPU error on R3F's first render tick
- Runtime shapers pass `{ features: [] }` to `stringToGlyphs` — eliminates whitespace collapse at wrap points caused by `liga`/`rlig` ligature substitution
- Advance-only glyphs (space, tab, zero-width) now emit proper entries at runtime, matching baked path advance resolution
- `SlugStackText.dispose()` properly tears down outline child meshes and `SlugStrokeMaterial` instances before disposing shared geometries
- `SlugFontLoader.clearCache` removed (cache keyed on `url:runtime?`, no-op call)

### Examples

- `examples/vanilla/slug-text` relocated to `examples/three/slug-text`; React + Three examples at 1:1 parity
- Icon-fallback demo with Font Awesome Solid PUA codepoints via `[Inter, FA-Solid]` stack; `[Lorem | Icons]` toggle in both examples
- Canvas2D compare overlay with onion / split / diff / off modes; draggable split handle
- Measure overlay with hover-to-measure: cyan ink bounds + dashed yellow font envelope per line
- `DprSync` component syncs R3F canvas pixel ratio after monitor swap or fullscreen transition
- Migrated from Web Awesome controls to `@three-flatland/tweakpane`

### BREAKING CHANGES

- Baked `.slug.bin` / `.slug.json` files must be re-generated with the updated `slug-bake` CLI (format version 2→4)

Phase 1–5 of the Slug text rendering library: measurement, decorations, font stacks, analytic stroke outline, and a complete quadratic-Bezier stroke bake pipeline — all with React and Three.js example parity.
