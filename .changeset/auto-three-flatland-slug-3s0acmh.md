...
- New `@three-flatland/slug` package: analytic GPU text rendering via TSL NodeMaterials on WebGPU
- `SlugText` — instanced glyph rendering with font parsing, text shaping, and band-accelerated fragment shader
- `SlugMaterial` — TSL NodeMaterial with analytic winding-number coverage via horizontal/vertical band lookup
- `SlugGeometry` — instanced glyph geometry with per-glyph texture references
- Dynamic instance quad dilation for sub-pixel anti-aliasing
- Stem darkening and thickening options on `SlugMaterial` and `SlugText`

## Baked font pipeline + CLI

- `slug-bake` CLI: converts OpenType fonts to `.slug.{json,bin}` format for zero-runtime-parse rendering
  - `--output / -o` for custom output bases
  - `--stroke-widths / --stroke-join / --stroke-cap / --miter-limit` flags for pre-baked stroke sets
- All APIs (`shapeText`, `wrapText`, `measureText`) dispatch to baked path without loading opentype.js at runtime

## Font loading + metrics

- `SlugFontLoader` — async font loader with URL-keyed cache
- `SlugFont.wrapText(text, fontSize, maxWidth?)` → `string[]` — line-break-aware wrap matching shaper output exactly
...
- `StyleSpan { start, end, underline?, strike? }` — underline and strikethrough span API
- `SlugText.styles` setter — decoration spans rendered in the same draw call via rect-sentinel instances
- Decoration metrics sourced from OpenType `post`/`os2` tables and baked into `.slug.json`

## Outline / stroke rendering

- `SlugStrokeMaterial` — TSL distance-to-curve stroke shader with analytic bevel-via-min joins
- `SlugText.outline: SlugOutlineOptions` — opt-in outline as a child `InstancedMesh` sharing fill geometry; `renderOrder = -1`
- `SlugText.setOutlineWidth(w)` / `setOutlineColor(c)` — runtime uniform setters, zero geometry rebuild
- `SlugText.setOpacity(v)` — fill opacity for outline-only compositing
- `SlugOutlineOptions` exported from package root
- Stroke shader compile cost halved; axis-aligned vertex expansion fixes stroke clipping at glyph extents
...
- `SlugStackText` — multi-font `Group` with one `InstancedMesh` per font; supports `styles`, `outline`, `setOpacity`, `dispose`
- `SlugFontStack.wrapText(text, fontSize, maxWidth?)` — wrap output matching stack shaper line breaks

## Pre-baked stroke sets

- `BakedJSON.strokeSets` — optional metadata array for pre-baked stroke pseudo-glyphs (width, join, cap, miterLimit, offset)
- `SlugFont.getStrokeGlyph(sourceId, width, join, cap, miterLimit?)` — look up a pre-baked stroke glyph
- `bakeStrokeForGlyph` — contour-offsetting pipeline from source glyph to packed GPU stroke glyph
- Full quadratic Bezier `strokeOffsetter`: adaptive subdivision, per-segment Tiller-Hanson offset, join insertion (bevel/miter/round), cap insertion (flat/square/round/triangle), contour stitching into closed annular rings
- Shared `buildGpuGlyph.ts` pipeline module consumed by fontParser, strokeOffsetter, and future SVG path support

## Performance

- Texture formats optimized: `curveTexture` → RGBA16F (half bandwidth), `bandTexture` → RG32F; `.slug.bin` ~45% smaller on disk
- `MAX_CURVES_PER_BAND` 64 → 40 (covers p999 of Inter's full glyph corpus with margin)
- `bandCount` 8 → 16, halving expected curves per band and per-fragment ALU cost
- Shader early-exits for curves that don't cross the fragment's ray (~30% fewer full evaluations)

## Bug fixes

- Fixed whitespace collapse at wrap points caused by opentype.js `liga`/`rlig` features shortening glyph arrays
- Fixed `SlugText` flipping visible before first rebuild, which caused WebGPU binding errors and a blank canvas
- Fixed baked `measureText` returning zero ink bounds for all glyphs (`curves.length` heuristic replaced with bounds-area check)
- Fixed stroke vertex expansion producing clipped corners (diagonal normal replaced with axis-aligned per-component expansion)
- Fixed pipeline stall on first outline-enable toggle (Newton seeds reduced from 3×3 to 1×3; compile time halved)
- Fixed `SlugStackText.dispose` leaving outline meshes and stroke materials un-cleaned

## BREAKING CHANGES

- **BAKED_VERSION 2 → 3**: `curveTexture` changed to RGBA16F, `bandTexture` to RG32F, `MAX_CURVES_PER_BAND` lowered to 40. All `.slug.{json,bin}` assets must be re-baked with the updated `slug-bake` CLI.
- **BAKED_VERSION 3 → 4**: Decoration metrics (`underlinePosition`, `underlineThickness`, `strikethroughPosition`, `strikethroughThickness`) added to `BakedJSON.metrics`. All `.slug.{json,bin}` assets must be re-baked.
- `BAKED_VERSION` compatibility-check machinery removed from `SlugFontLoader` (package was pre-release; no migration path maintained).

This release ships the full `@three-flatland/slug` package: analytic GPU text rendering from initial pipeline through measurement, decorations, multi-font stacks, outline/stroke, and a complete quadratic Bezier pre-bake workflow — two BAKED_VERSION bumps require re-baking any existing `.slug.{json,bin}` assets.
