...
- `SlugText.outline` — opt-in child stroke mesh with runtime-uniform `width` and `color`; enable/disable without rebuilding geometry
- `SlugText.setOutlineWidth(v)` / `setOutlineColor(v)` — zero-rebuild uniform setters
- `SlugText.setOpacity(v)` — fill-only opacity control enabling outline-only mode
- New `SlugStrokeMaterial`: TSL analytic distance-to-quadratic-Bezier stroke material, bevel-via-min joins
- Fixed stroke quad clipping: axis-aligned vertex expansion replaces diagonal expansion, eliminating squared-off outer stroke corners
- Halved stroke shader compile cost (single Newton seed instead of three), eliminating first-draw hitch on outline enable
- `SlugOutlineOptions` exported from package root

### SlugStackText parity

- `SlugStackText.outline` — per-font stroke meshes matching `SlugText.outline` API
- `SlugStackText.styles` — underline/strikethrough span support in font-stack mode
- `SlugStackText.setOpacity(v)` — forwards to all per-font fill materials
- `SlugStackText.dispose()` now correctly tears down outline meshes before disposing shared geometries

### Measurement API

- `SlugFont.measureText(text, fontSize)` → `TextMetrics` — single-line metrics aligned with `CanvasRenderingContext2D.measureText`
- `SlugFont.measureParagraph(text, fontSize, { maxWidth?, lineHeight? })` → `ParagraphMetrics` — multi-line convenience over `wrapText`; respects same `lineHeight` default (1.2) as `SlugText`
- Baked measure uses `bounds` area gate instead of `curves.length` to correctly report ink bounds for baked glyphs

### Font stack / text shaping

- `SlugFontStack.wrapText(text, fontSize, maxWidth?)` → `string[]` — per-codepoint font resolution with same word-break policy as `shapeStackText`
- `SlugFont.hasCharCode(codepoint)` — codepoint coverage check for per-codepoint fallback routing
- `SlugFontStack.emitDecorations()` — per-glyph advance lookup for stacked font runs
- Runtime shapers pass `{ features: [] }` to `stringToGlyphs`, preventing ligature substitution from collapsing word-boundary indices
- `parseFont` emits advance-only entries for cmap'd glyphs with no outline (space, tab, zero-width controls)

### Bake / CLI

- `slug-bake` gains `--stroke-widths`, `--stroke-join`, `--stroke-cap`, `--miter-limit` — bakes stroke pseudo-glyphs into output at `glyphIdOffset + sourceId`
- `BakedJSON.strokeSets` optional field; backward-compatible (absent when not configured)
- `SlugFont.getStrokeGlyph(sourceId, width, join, cap, miterLimit?)` — look up pre-baked stroke `SlugGlyphData`
- `slug-bake` gains `--output` / `-o` for custom output path base
- `BAKED_VERSION` machinery removed (pre-release, no migration story)
- Kerning extraction filtered to source IDs only to avoid resolving out-of-range stroke glyph IDs

### Stroke offsetter pipeline (internal)

- New `strokeOffsetter` module: adaptive subdivision, Tiller-Hanson per-segment offset, join insertion (bevel/miter/round with `miterLimit` fallback), cap insertion (flat/square/round/triangle), contour stitching into closed shapes
- New `bakeStrokeForGlyph` helper composing the offsetter into `SlugGlyphData`
- Extracted `buildGpuGlyphData` / `buildGpuGlyphFromCurves` / `buildAdvanceOnlyGlyph` shared pipeline module (used by fontParser, stroke offsetter, and future SVG path support)

### Bug fixes

- `SlugText._setFont` defers `visible = true` until first `_rebuild`, preventing zero-binding WebGPU frame rejection on R3F prop-set
- Compare overlay in examples uses `stack.wrapText` so line breaks match `SlugStackText` at any `maxWidth`

Adds analytic text stroke rendering (`SlugText.outline`, `SlugStrokeMaterial`), full-stack outline parity for `SlugStackText`, a complete quadratic-Bézier stroke offsetter with CLI bake integration, `measureText`/`measureParagraph` APIs, and `SlugFontStack.wrapText` for multi-font line breaking.
