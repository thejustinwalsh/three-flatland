...
- `SlugFont.measureText(text, fontSize)` — single-line text metrics aligned with `CanvasRenderingContext2D.measureText` (width, actualBoundingBox*, fontBoundingBox*)
- `SlugFont.measureParagraph(text, fontSize, { maxWidth?, lineHeight? })` — multi-line paragraph metrics; respects same `lineHeight` default as `SlugText`
- `SlugFont.wrapText(text, fontSize, maxWidth?)` — wrap to string[] using same break policy as `shapeText`; dispatches baked vs runtime
- `SlugFont.hasCharCode(c)` — codepoint coverage check via cmap
- `SlugFont.getStrokeGlyph(sourceId, width, join, cap, miterLimit?)` — look up a pre-baked stroke glyph from a `strokeSet`
- `SlugFontStack(fonts)` — per-codepoint fallback chain; `resolveCodepoint`, `resolveText`, `wrapText`, `emitDecorations`
- `SlugStackText` — multi-font `Group` renderable; one `InstancedMesh` per contributing font
- `SlugStackText.styles`, `.outline`, `.setOpacity()`, `.setOutlineWidth()`, `.setOutlineColor()` — parity with `SlugText`
- `SlugText.outline` — opt-in stroke child mesh (shared geometry, `SlugStrokeMaterial`, `renderOrder -1`)
- `SlugText.setOpacity(value)`, `.setOutlineWidth()`, `.setOutlineColor()` — runtime-uniform setters, zero rebuild
- `StyleSpan { start, end, underline?, strike? }` — text decoration span; applied via `SlugText.styles` / `SlugStackText.styles`
- `SlugStrokeMaterial` — stroke-capable `NodeMaterial`; distance-to-curve analytic fragment shader
- `SlugOutlineOptions` — exported type for outline configuration
- `buildGpuGlyphData` / `buildGpuGlyphFromCurves` / `buildAdvanceOnlyGlyph` — shared contour-to-GPU pipeline, used by font parser, stroke offsetter, and future SVG path support

### slug-bake CLI

- `--stroke-widths`, `--stroke-join`, `--stroke-cap`, `--miter-limit` — bake pre-computed stroke pseudo-glyphs into `.slug.{json,bin}`; stored in `BakedJSON.strokeSets` (optional, backward-compatible)
- `--output / -o` — custom output base path
- Warning when any band exceeds `MAX_CURVES_PER_BAND` shader bound

### Performance

- Texture format: `curveTexture` → `RGBA16F`, `bandTexture` → `RG32F` — ~45% smaller `.slug.bin`, ~20% GPU time reduction
- `bandCount` 8 → 16; `MAX_CURVES_PER_BAND` 64 → 40 (covers 100% of Inter corpus)
- Shader skips sqrt/coverage work for non-crossing curves (`If(rootCode > 0)`)
- Stroke shader: single Newton seed at `t=0.5` + endpoints — halves WGSL compile time and per-fragment cost

### Bug Fixes

- `SlugText._setFont`: mesh stays hidden until first `_rebuild` completes — prevents blank WebGPU canvas on first R3F render
- `SlugStackText.dispose()`: tears down outline meshes and fill `InstancedMesh`es before disposing shared geometries — fixes GPU leak on scene toggle
- Runtime shapers pass `{ features: [] }` to `opentype.js` — prevents `liga`/`rlig` ligature substitution from collapsing wrap-boundary characters and producing wrong line breaks
- `parseFont` emits advance-only entries for space/tab/zero-width codepoints at runtime, matching the bake CLI's post-pass
- `SlugStrokeMaterial`: quad expansion is axis-aligned (`strokeHalfWidth` per axis) before the AA dilation pass — fixes stroke clipping at glyph x/y extents
- Kerning extraction filters to source glyph IDs only — prevents crash when stroke glyph IDs enter `opentype.js` kern resolver

### BREAKING CHANGES

- **BAKED_VERSION 2 → 3**: `curveTexture` and `bandTexture` format change; re-run `slug-bake` on all `.slug.{json,bin}` assets
- **BAKED_VERSION 3 → 4**: decoration metrics added to `BakedJSON.metrics`; re-run `slug-bake` on all `.slug.{json,bin}` assets
- `SlugFontLoader.clearCache` removed (cache is already keyed on `url:runtime?`, no migration needed)
- `BAKED_VERSION` machinery removed from `SlugFontLoader` — no migration story for unreleased builds
...
This release introduces the full `@three-flatland/slug` library: analytic WebGPU text rendering with a `slug-bake` CLI, runtime font stacks with per-codepoint fallback, text measurement, underline/strikethrough decorations, runtime-uniform outline/stroke, and a complete quadratic-Bezier stroke offsetter for baking pre-computed stroke glyphs.
