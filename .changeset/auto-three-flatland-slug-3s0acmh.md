...
- `SlugFont.measureParagraph(text, fontSize, { maxWidth?, lineHeight? })` → `ParagraphMetrics` — multi-line block metrics matching SlugText render dimensions
- `SlugFont.wrapText(text, fontSize, maxWidth?)` → `string[]` — wrap lines matching shaped output exactly
- `SlugFont.hasCharCode(c)` — codepoint coverage check for fallback routing
- `SlugFont.getStrokeGlyph(sourceId, width, join, cap, miterLimit?)` — look up a pre-baked stroke glyph by parameters
- `SlugFont.emitDecorations(text, positioned, styles, fontSize)` — build decoration rects from StyleSpan list
- `SlugFontStack(fonts)` — ordered per-codepoint font fallback chain; `resolveCodepoint`, `resolveText`, `wrapText`, `emitDecorations`
- `SlugStackText` — multi-font Group renderable, one InstancedMesh per font; supports `styles`, `outline`, `setOpacity`, `dispose`
- `SlugText.outline` — opt-in stroke outline (`SlugOutlineOptions`), child mesh sharing fill geometry
- `SlugText.setOpacity(value)`, `setOutlineWidth(v)`, `setOutlineColor(v)` — runtime-uniform setters, no rebuild
- `SlugText.styles` — `StyleSpan[]` for underline / strikethrough decorations
- `SlugStrokeMaterial` — stroke NodeMaterial (exported from package root); `SlugOutlineOptions` type exported alongside
- `StyleSpan { start, end, underline?, strike? }` — decoration span type
- `TextMetrics` / `ParagraphMetrics` types exported from package root
- `slug-bake` CLI: `--output / -o` for custom output path; `--stroke-widths`, `--stroke-join`, `--stroke-cap`, `--miter-limit` flags bake pre-computed stroke glyphs into the binary
- Baked format: optional `BakedJSON.strokeSets` array — fonts baked without stroke flags load unchanged

## Performance

- `curveTexture` switched to RGBA16F — half bandwidth vs RGBA32F
- `bandTexture` switched to RG32F — half bandwidth vs RGBA32F
- `MAX_CURVES_PER_BAND` lowered 64 → 40 (Inter full corpus p999 = 25, max = 38); reduced shader register pressure
- Band count doubled 8 → 16 — expected curves per band ~halved, linear reduction in fragment ALU
- Shader: wrap post-rootCode path in `If(rootCode > 0)` — ~30% of curves skip sqrt/divisions entirely
- Stroke shader: single Newton seed with 3 iterations (was 3 seeds × 3 each); cuts WGSL ~50%, faster pipeline compile and per-fragment cost

## Bug Fixes

- Stroke quad outer ring no longer clips at glyph extents — expansion now axis-aligned in vertex shader rather than along diagonal unit normal
- First outline-enable lag eliminated — smaller stroke WGSL compiles ~2× faster on first use
- `SlugStackText.dispose()` now tears down outline meshes and fill InstancedMeshes before geometry disposal; eliminates GPU leaks on scene toggle
- Runtime shapers now pass `{ features: [] }` to `stringToGlyphs` — prevents opentype.js `liga`/`rlig` from collapsing whitespace at wrap points
- `SlugText` defers `visible = true` until after first `_rebuild` — prevents WebGPU "Binding size is zero" error when R3F renders before `useFrame` fires
- `SlugText._setFont` only rebuilds outline when outline is already enabled — avoids GPU resource cost for users who never opt in

## BREAKING CHANGES

- **BAKED_VERSION 2 → 3**: curve texture format changed to RGBA16F, band texture to RG32F, MAX_CURVES_PER_BAND lowered to 40. All `.slug.bin` / `.slug.json` files must be re-baked with the updated CLI.
- **BAKED_VERSION 3 → 4**: decoration metrics (underline/strikethrough positions and thicknesses) added to `BakedJSON.metrics`. All baked files must be re-baked.
- `SlugFontLoader.clearCache` removed — cache is already keyed per URL, no migration path
- `BAKED_VERSION` machinery removed from `SlugFontLoader`
- `slugDilate`'s `strokeHalfWidth` parameter removed — stroke expansion is now handled axis-aligned in `SlugStrokeMaterial`'s vertex shader; fill-only callers are unaffected

## Examples

- `examples/vanilla/slug-text` relocated to `examples/three/slug-text`; package renamed to `example-three-slug-text`
- React example updated to full parity with Three example: Canvas2D compare overlay with onion / split / diff modes, draggable split handle, diff heatmap
- Icon fallback demo: FontAwesome Solid PUA codepoints baked with `slug-bake`, [Inter, FA-Solid] stack in both examples
- Measure overlay: hover any rendered line to see cyan ink bounds and dashed yellow font envelope with live monitors
- Styles folder: underline / strikethrough demo via `StyleSpan` API
- Outline controls: fill / outline / both radio, live width slider, color picker
