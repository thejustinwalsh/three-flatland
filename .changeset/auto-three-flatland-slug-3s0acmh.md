...
- `SlugFont.measureParagraph(text, fontSize, { maxWidth?, lineHeight? })` — multi-line block metrics matching SlugText's layout
- `SlugFont.wrapText(text, fontSize, maxWidth?)` — line-break array matching the shaped output exactly
- `SlugFont.hasCharCode(c)` — fast codepoint-coverage check via the font's cmap
...
- `SlugStackText.styles` — underline/strike spans across mixed-font runs
- `SlugStackText.outline` — per-font stroke meshes with runtime `setOutlineWidth` / `setOutlineColor`
- `SlugStackText.setOpacity(value)` — per-font fill opacity for outline-only mode
- `SlugText.outline` — opt-in child stroke mesh; `setOutlineWidth`, `setOutlineColor`, `setOpacity` runtime setters; `SlugOutlineOptions` exported from package root
- `SlugStrokeMaterial` — distance-to-curve NodeMaterial for analytic stroke rendering; exported from package root
- `StyleSpan { start, end, underline?, strike? }` — text decoration spans for SlugText and SlugStackText
- `SlugFont.emitDecorations` — decoration rect emission using font-declared underline/strike metrics
- `slug-bake` CLI: `--output / -o` flag for custom output base path; `--stroke-widths / --stroke-join / --stroke-cap / --miter-limit` flags for baking pre-offset stroke glyph sets
- `SlugFont.getStrokeGlyph(sourceId, width, join, cap, miterLimit?)` — look up a pre-baked stroke glyph by set parameters
- `buildGpuGlyphData` / `buildGpuGlyphFromCurves` / `buildAdvanceOnlyGlyph` — shared pipeline builders for font parser, SVG paths, and stroke offsetter

## Stroke Offsetter Pipeline (Phase 5)

- Quadratic-Bezier stroke offsetter: adaptive subdivision, per-segment Tiller-Hanson offset, join insertion (bevel / miter with fallback / round), cap insertion (flat / square / triangle / round), contour stitching into closed offset contours
- `bakeStrokeForGlyph(source, options)` — bridge from offsetter output to `SlugGlyphData` for CLI bake and runtime fallback paths
- Baked format gains optional `strokeSets` field; absent for fonts baked without stroke flags — backward compatible

## Performance

- `curveTexture` → RGBA16F (half the bandwidth); `bandTexture` → RG32F; `MAX_CURVES_PER_BAND` 64 → 40 based on Inter corpus analysis
- Band count 8 → 16 halves expected curves per band (~6 → ~3 mean); fragment shader skips non-crossing curves with an early-out branch
- `distanceToQuadBezier` shader reduced from 3 Newton seeds × 5 candidates to 1 seed + 2 endpoints — halves WGSL compile time and runtime fragment cost
- Stroke quad expansion moved to axis-aligned vertex shader pass, fixing clipped outer ring at glyph extents
- `SlugText._setFont` skips outline rebuild when outline is not enabled

## Rendering Fixes

- `SlugText` visibility deferred until first `_rebuild` with real glyph data — prevents zero-size buffer pipeline error on R3F's initial render pass
- `SlugStackText.dispose()` now tears down outline meshes and fill `InstancedMesh` children in the correct order
- `parseFont` emits advance-only entries for cmap'd glyphs with no outline (space, tab, zero-width controls) — matches baked CLI output
- Runtime shapers pass `{ features: [] }` to `stringToGlyphs` — disables `liga`/`rlig` that collapsed whitespace tokens and broke word-boundary detection
- `bakeStrokeForGlyph` kerning extraction now filters to source IDs only — prevents `_push is not a function` on stroke glyph ID ranges

## BAKED_VERSION History

- v2 → v3: `curveTexture` RGBA16F, `bandTexture` RG32F, `MAX_CURVES_PER_BAND` 40; included fixtures re-baked
- v3 → v4: decoration metrics (`underlinePosition`, `underlineThickness`, `strikethroughPosition`, `strikethroughThickness`) added to `BakedJSON.metrics`
- `BAKED_VERSION` machinery subsequently removed from `SlugFontLoader` (pre-release; no migration story needed)

## Examples (React + Three, 1:1 parity)

- Canvas2D comparison overlay with onion-skin / split / diff modes and draggable split handle
- Measure folder: hover-to-select line shows cyan ink and dashed yellow font-envelope overlays with live tweakpane monitors
- Styles folder: underline/strike applied to preset character scopes
- Outline folder: Fill / Outline / Both radio, runtime width slider, color picker
- Lorem / Icons radio: icons mode loads FA-Solid PUA glyphs baked with `slug-bake` against a `[Inter, FA-Solid]` stack; Canvas2D compare switches font-family to match
- Compare mode gains `Off` option to hide overlay for standalone rendering or screenshots
- `<DprSync>` R3F component keeps pixel ratio in sync on monitor swap, OS zoom, and fullscreen transitions
- Migrated from Web Awesome to `@three-flatland/tweakpane` for all pane controls

This release ships the complete analytic stroke rendering surface (Phase 4), the font-stack fallback chain and `SlugStackText` (Phase 3), text decoration rendering (Phase 2), measurement APIs (Phase 1), and the Phase 5 stroke offsetter pipeline used for pre-baked stroke glyph sets.
