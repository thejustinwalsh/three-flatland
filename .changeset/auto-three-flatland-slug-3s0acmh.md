...
- **Initial package**: GPU text rendering pipeline — font parsing, text shaping, band/texture packing, TSL shaders; WebGPU-only
- **Baked font support**: `slug-bake` CLI produces `.slug.{json,bin}` asset pairs; `SlugFontLoader` loads baked or runtime fonts transparently
- **`SlugFont.wrapText(text, fontSize, maxWidth?)`**: returns line-wrapped string array via baked or runtime path; used by examples' Canvas2D overlay for line-break parity
- **`SlugFont.measureText` / `measureParagraph`**: single-line metrics aligned with `CanvasRenderingContext2D.measureText`; multi-line paragraph block dimensions
- **`StyleSpan` API**: underline and strikethrough decorations; spans attached to `SlugText.styles`; rect sentinels rendered in the same draw call
- **`SlugFontStack`**: ordered per-codepoint font fallback chain; `hasCharCode` coverage check; `resolveCodepoint` / `resolveText` helpers
- **`SlugStackText`**: multi-font renderable extending `Group`; one `InstancedMesh` per font; styles, outline, opacity parity with `SlugText`
- **`SlugFontStack.wrapText`**: stack-aware line wrapping so Canvas2D overlays stay line-for-line with `SlugStackText` output
- **`SlugText.outline`** / **`SlugStrokeMaterial`**: opt-in analytic stroke rendered via a child `InstancedMesh` sharing fill geometry; runtime `setOutlineWidth` / `setOutlineColor` uniforms
- **Stem darkening and thickening options** on `SlugMaterial` and `SlugText`
- **`slug-bake --stroke-widths/--stroke-join/--stroke-cap/--miter-limit`**: bakes stroke pseudo-glyphs into the same curve/band textures; `SlugFont.getStrokeGlyph()` retrieves them at runtime
- **`slug-bake --output/-o`**: custom output path base
- **Quadratic-Bezier stroke offsetter** (`pipeline/strokeOffsetter`): adaptive subdivision, per-segment offset, join insertion (miter/bevel/round), cap insertion (flat/square/triangle/round), contour stitching; `bakeStrokeForGlyph` helper bridges to the bake pipeline
- **`buildGpuGlyphData` / `buildGpuGlyphFromCurves`**: shared contour-to-GPU pipeline factored out of `fontParser` for reuse by stroke offsetter and future SVG path support

## Performance

- `curveTexture` → `RGBA16F` (half bandwidth); `bandTexture` → `RG32F`
- `MAX_CURVES_PER_BAND` 64 → 40 (covers 100% of Inter corpus with margin)
- `bandCount` 8 → 16; halves expected curves per band (~6.3 → ~3.2 mean)
- Shader: skip post-solve work on curves that don't cross the ray (~30% of curves in a band)
- Stroke shader: single Newton seed at `t=0.5` with endpoint candidates; halves WGSL size and pipeline compile time

## Bug fixes

- Stroke quads clipped at glyph extents fixed: expansion is now axis-aligned per vertex before the AA dilation pass
- `SlugText._setFont` no longer sets `visible=true` before the first `_rebuild`; prevents WebGPU "Binding size is zero" rejection on early R3F render pass
...
- `SlugStackText.dispose()` now tears down outline meshes and `SlugStrokeMaterial` before disposing shared geometries; eliminates GPU leaks on scene toggle
- Kerning extraction filters to source glyph IDs only; avoids `_push is not a function` when stroke ID ranges are present

## Breaking changes

- **`BAKED_VERSION` 2 → 3 → 4**: existing `.slug.bin` / `.slug.json` files must be re-baked with the current `slug-bake` CLI
- **`SlugFontLoader.BAKED_VERSION` machinery removed**; no migration story was needed pre-release
- **`SlugFontLoader.clearCache` removed**; the static cache is already keyed on `url:runtime?`

`@three-flatland/slug` ships a complete GPU-accelerated text rendering pipeline with baked-font tooling, analytic outlines with join/cap geometry, per-codepoint font-stack fallback, text decorations, and measurement APIs.
