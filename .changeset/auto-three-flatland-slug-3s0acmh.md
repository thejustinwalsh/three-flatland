...
**Measurement**
- `SlugFont.measureText(text, fontSize)` → `TextMetrics` — single-line metrics aligned with `CanvasRenderingContext2D.measureText` (width, actual/font bounding boxes)
...
- `SlugFont.wrapText(text, fontSize, maxWidth?)` → `string[]` — wraps text using the same break policy as the shaped output; dispatches baked vs runtime path

**Font stacks**
- `SlugFontStack(fonts)` — per-codepoint fallback chain; first covering font wins; uncovered codepoints fall back to the primary's notdef
- `SlugFontStack.wrapText(text, fontSize, maxWidth?)` → `string[]` — stack-aware wrapping so external renderers (Canvas2D overlays) stay line-for-line with `SlugStackText`
- `SlugFont.hasCharCode(codepoint)` — cheap codepoint coverage check via the font's cmap

**Multi-font rendering**
- `SlugStackText` — `Group` subclass with one `InstancedMesh` per font in the stack; one draw call per contributing font
- `SlugStackText.styles: StyleSpan[]` — underline/strikethrough spans; decoration line anchored to the primary font's metrics
- `SlugStackText.outline: SlugOutlineOptions` — per-font stroke `InstancedMesh` sharing the fill mesh's `instanceMatrix`; `setOutlineWidth`/`setOutlineColor` are uniform-only
- `SlugStackText.setOpacity(value)` — forwards to all per-font fill materials

**Text decorations**
- `StyleSpan { start, end, underline?, strike? }` — range-based underline/strikethrough API
- `SlugText` and `SlugStackText` accept `styles?: StyleSpan[]` at construction and as a runtime setter

**Outline rendering**
- `SlugText.outline` — opt-in child `InstancedMesh` (renderOrder -1) backed by `SlugStrokeMaterial`; shares fill geometry, no rebuild on width/color change
- `SlugText.setOutlineWidth(v)`, `SlugText.setOutlineColor(v)` — runtime-uniform setters, zero rebuild
- `SlugText.setOpacity(value)` — fill material opacity for outline-only mode
- `SlugOutlineOptions` and `SlugStrokeMaterial` exported from the package root

**Stroke bake pipeline**
- `slug-bake` gains `--stroke-widths`, `--stroke-join`, `--stroke-cap`, `--miter-limit` flags to pre-bake stroke pseudo-glyphs into `.slug.{json,bin}`; `--output / -o` for custom output base
- `SlugFont.getStrokeGlyph(sourceId, width, join, cap, miterLimit?)` — looks up a pre-baked stroke glyph by set parameters
- `bakeStrokeForGlyph(source, options)` — converts a source glyph into a stroked `SlugGlyphData` via the offsetter
- `strokeOffsetter(curves, closed, options)` — complete quadratic-Bezier stroke offsetter: adaptive subdivision, per-segment Tiller-Hanson offset, miter/bevel/round joins, flat/square/triangle/round caps, closed and open contour stitching
- `buildGpuGlyphData`, `buildGpuGlyphFromCurves`, `buildAdvanceOnlyGlyph` — shared contour-to-GPU pipeline helpers (factored from `fontParser`, reused by stroke bake and future SVG path support)

## Bug Fixes

- Stroke outline clipping fixed: quad expansion is now axis-aligned per axis (`strokeHalfWidth` along X and Y independently) instead of along the diagonal unit normal, eliminating the squared-off clipping at glyph extents
- Stroke shader compile cost halved: reduced Newton seeds from 3×3 to 1×3 (single seed at t=0.5 + both endpoints), directly halving WGSL size and first-frame pipeline compile stall
- `SlugText._setFont` no longer sets `visible=true` before the first `_rebuild` — prevents blank canvas on R3F when a pipeline compile fires against an uninitialized instance buffer
- Runtime shapers now pass `{ features: [] }` to `stringToGlyphs`; eliminates opentype.js `liga`/`rlig` substitution that was collapsing whitespace at wrap boundaries
- `parseFont` emits advance-only glyph entries for whitespace/zero-width cmap'd glyphs, matching the bake CLI's post-pass
- `SlugStackText.dispose()` now fully tears down outline meshes before fill meshes to avoid double-free and GPU leaks on scene toggle
- Kerning extractor filters to source IDs only — stroke pseudo-glyph IDs no longer trigger `this.font._push is not a function`
- Baked `measureText` now gates ink bounds on `xMax > xMin` instead of `curves.length > 0`; previously returned zero ink bounds for all baked glyphs
- `BAKED_VERSION` machinery removed from `SlugFontLoader` — no migration gating on an unreleased package

## Performance

- `bandCount` 8 → 16: halves mean curves per band (~6.3 → ~3.2), proportionally reducing fragment ALU in the hot loop
- Shader skips the root solve, coverage, and weight work for non-crossing curves (`If(rootCode > 0)`) — ~30% of band curves are skipped on typical text
- `curveTexture` changed to `RGBA16F` (HalfFloatType): 8 bytes/texel instead of 16; em-space coords fit comfortably in 11-bit mantissa
- `bandTexture` changed to `RG32F`: eliminates two wasted float channels per texel
- `MAX_CURVES_PER_BAND` reduced 64 → 40 (p999 of Inter's full glyph corpus is 25; 40 covers 100% with margin); lowers shader register pressure

> **Note:** `.slug.bin`/`.slug.json` files must be regenerated with the current `slug-bake` CLI due to texture format and band metric changes. Re-baked fixtures are ~45% smaller on disk.

## Examples

- Both Three.js and React examples maintain 1:1 parity throughout; all features land in both simultaneously
- Canvas2D comparison overlay with onion-skin, split, and diff modes; draggable split handle
- `[Lorem | Icons]` radio toggle: `lorem` renders `SlugText`, `icons` renders `SlugStackText` against an `[Inter, FA-Solid]` stack with a 12-icon FA-Solid subset (~71 KB baked bin)
- Compare overlay uses `stack.wrapText` in icons mode for line-break agreement at any `maxWidth`
- Measure overlay: hover a rendered line to see cyan ink bounds + dashed yellow font envelope; click to lock
- Styles folder: underline/strikethrough scope presets (first word / first sentence / first line)
- Outline folder: Fill / Outline / Both radio + live width slider + color picker
- Renderer set to `antialias: false` — Slug computes analytic per-fragment coverage; MSAA is 4× cost for zero visual gain

This release delivers the full Phase 1–5 text rendering feature set for `@three-flatland/slug`: measurement, decorations, multi-font stacks, analytic outlines with runtime controls, and a complete stroke bake pipeline.
