...
Analytic GPU text rendering for Three.js and React Three Fiber using WebGPU + TSL (no GLSL, no WebGL).

### Core rendering pipeline
- `SlugFont`, `SlugText`, `SlugGeometry`, `SlugMaterial` — load a font, shape text, render per-fragment analytic coverage via winding-number TSL shader
- `SlugFontLoader` — async font loader with static URL-keyed cache; dispatches baked vs runtime paths transparently
- Instanced rendering: one draw call per font per frame regardless of glyph count
- R3F subpath (`three-flatland/react`) with matching component types

### Baked font format + CLI
- `slug-bake` CLI — converts a TrueType/OpenType font to `.slug.{json,bin}`; flags: `--subset`, `--stroke-widths`, `--stroke-join`, `--stroke-cap`, `--miter-limit`, `--output / -o`
- Baked path keeps opentype.js out of the browser bundle; runtime path loads `.ttf`/`.otf` lazily
- `BakedJSON.strokeSets` — optional pre-baked stroke contours embedded in the same curve/band textures (baked stroke renders via the fill shader at 1× cost)
...
- `SlugFont.wrapText(text, fontSize, maxWidth?)` → `string[]` — line-break output matches shaped render

### Text decorations
- `StyleSpan { start, end, underline?, strike? }` — character-range decoration spans
- `SlugText.styles` setter applies underline/strikethrough in the same draw call via rect-sentinel instances in `SlugGeometry`

### Font stack + multi-font rendering
- `SlugFontStack` — ordered fallback chain; `resolveCodepoint(c)` walks fonts, first coverage wins; `wrapText` respects per-codepoint resolution; `hasCharCode` for cheap coverage checks
- `SlugStackText extends Group` — one `InstancedMesh` per font in the stack; styles, outline, and `setOpacity` on par with `SlugText`
- `SlugFontStack.emitDecorations()` — decoration lines use primary font metrics across mixed-font runs

### Text outline (runtime-uniform stroke)
- `SlugText.outline: SlugOutlineOptions` — opt-in child `InstancedMesh` sharing the fill geometry; `setOutlineWidth`, `setOutlineColor` update uniforms with zero rebuild
- `SlugText.setOpacity(value)` — fade fill independently for outline-only mode
- `SlugStrokeMaterial` — distance-to-curve fragment shader; axis-aligned quad expansion before AA dilation pass; shader compile cost halved (single Newton seed + endpoints)
- `SlugOutlineOptions.color` accepts `number | string | Color`

### Phase 5 stroke geometry (bake-time)
- Quadratic-Bézier stroke offsetter: adaptive subdivision, per-segment Tiller-Hanson offset, bevel/miter/round join insertion, flat/square/triangle/round cap insertion, contour stitching into closed annular rings
- `bakeStrokeForGlyph(source, options)` — runs offsetter over font contours, packs result into `SlugGlyphData` via `buildGpuGlyphData`
- `SlugFont.getStrokeGlyph(sourceId, width, join, cap, miterLimit?)` — look up a pre-baked stroke glyph

### Performance
- Curve texture → `RGBA16F` (8 bytes/texel, was 16); band texture → `RG32F`; `MAX_CURVES_PER_BAND` 64 → 40 (covers Inter p999 with margin)
- Band count 8 → 16: ~halves expected curves/band in the hot fragment loop
- Shader: skip winding-number solve for non-crossing curves (`If(rootCode > 0)`)

### Pipeline refactors
- `buildGpuGlyphData` / `buildGpuGlyphFromCurves` / `buildAdvanceOnlyGlyph` — shared contour-to-GPU factory used by fontParser, strokeOffsetter, and future SVG path producer
- Advance-only glyph entries (space, tab, zero-width controls) emitted at runtime to match baked path advances
- Runtime shapers pass `{ features: [] }` to opentype.js to prevent `liga`/`rlig` from collapsing word-boundary checks
- `SlugText._setFont` defers `visible = true` until first `_rebuild` to prevent WebGPU zero-size binding errors on R3F's first render pass
...
- Tweakpane Settings + Styles + Measure + Outline folders with live-updating monitors
- [Lorem | Icons] scene toggle; Icons mode loads a 12-icon FA-Solid subset baked with `slug-bake`
- Compare overlay uses `stack.wrapText` in icons mode so line breaks agree with `SlugStackText`
- Click-to-measure: select a rendered line for cyan ink bounds + yellow font-envelope overlays
- Compare mode `Off` hides the overlay entirely; measure overlay still available in Lorem mode

### BREAKING CHANGES
- `BAKED_VERSION` bumped 2 → 3 → 4; any `.slug.bin`/`.slug.json` files baked with a prior release must be re-generated with `slug-bake`
- `SlugFontLoader.clearCache` removed (cache is already URL-keyed; no migration needed)
- `BAKED_VERSION` machinery removed from `SlugFontLoader` (package was pre-release)

New analytic GPU text rendering package for Three.js/R3F with WebGPU+TSL, covering font loading, text shaping, measurement, decorations, multi-font stacks, runtime outlines, and baked stroke geometry.
