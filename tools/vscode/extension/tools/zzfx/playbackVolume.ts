// Extension-side read of the `threeFlatland.zzfx.playbackVolume` trim —
// thin vscode-config glue over the SHARED dB→multiplier mapping in
// webview/zzfx/volumeTrim.ts (same webview-module import precedent as
// lm/core.ts's params.ts import). Both play paths — the inline sidecar
// (register.ts → zzfx-play's `volume` command field) and the tuner
...
import { trimToMultiplier } from '../../../webview/zzfx/volumeTrim'

export const PLAYBACK_VOLUME_SETTING = 'threeFlatland.zzfx.playbackVolume'
